...
> | **L7** | ✅ **KOMPLETT 2026-07-26** (L7a + L7b-1 + L7b-2a + L7b-2b) | L7a: `RecipeOneShotService::anreichern()` + `BulkEnrichService::ZIELFELDER`/`luecken()`/`laufAnlegen()`; `voll_anreichern` an `recipes.GENERATE`. L7b-1: **V-035 entschieden** — `VkGeneratorModal` von synchron auf `GenerateRecipeJob` gehoben, Lauf-Mechanik im geteilten Trait `HatGeneratorLauf`, Toggle „⚡ Voll anreichern" (Default AN) + Ergebnis-Zeile als geteilte Blade-Komponenten an **beiden** Flächen, Job-Timeout 300→540 s bei aktiver Kaskade. L7b-2a: Kohärenz-Glied (`RecipeOneShotService::kohaerenzGlied` = `CoherenceService::judge` als GL-10-Achse 2, nur VK ab 2 Komponenten, eigenes try/catch, läuft auch ohne Anreicherungs-Lücke) + Sub-Rezept-Stub-Flag (`statistik['stubs']` mit Namen; dauerhafter Flag bleibt Signal `rezept_sub_stub_offen`), beide Zeilen an beiden Flächen. L7b-2b: Hard-Stops sind Wege statt Text — `HardstopResolveService` (Ein-Zeilen-Pfad: Row-Update + genau ein `recomputeAndPropagate`, Guards wie im Voll-Sync) + Trait `HatHardstopAktionen` + geteilte Blade `hardstop-zeilen`: „🧩 Basisrezept anlegen" legt an und verknüpft, „🤔 Meintest du?" bindet einen Shortlist-Kandidaten als Override, „📥 Beschaffung anstoßen" schreibt einen Sourcing-Wunsch OHNE GP (LA-First) und lässt die Zeile offen. **L7 damit gebaut bis auf L8** |
> | **L8** | ✅ **KOMPLETT 2026-07-27** (L8a + L8b-1 + L8b-2) — damit ist Phase 4 abgeschlossen. **L8b-2:** Ziel-VK-Feld am VK-Generator-Modal, wirkt zweifach über EINEN Transportweg (`parameter.ziel_vk_eur`): Prompt-Constraint in `vk.generator` („bau auf den Preis hin, GIB KEINEN PREIS AUS") + Abgleich nach der Kalkulation. **Keine eigene Toleranz-Schwelle** — `ziel_wareneinsatz_pct` ist derselbe Bruch mit dem Ziel-VK im Nenner, beurteilt von derselben Ampel-Leiter; `ziel_delta_eur` = Ist − Ziel; der Preis wird nie gedrückt (kein Solver). Band 0,50–500 € mit Rückfrage statt stiller Verwerfung (Absender ist ein Mensch — Gegenstück zu `portionG`), MCP `ziel_vk` nur mit `vk=true`. | L8a: `RecipeOneShotService::wirtschaftlichkeitsGlied()` am Ende der Kaskade (nur VK) — Default-AK Klasse-vor-HG + `ensureStandard` + Auto-VK aus der bestehenden Darreichungs-Maschine (`price_mode='auto'`, kein Auto-Publish) + W%-Ampel/Signal über das herausgezogene `SignalDetektorService::wareneinsatzUeberZielFuer()`; `recipes.GENERATE` liefert `sales_net` + `anreicherung.wirtschaftlichkeit`. Portion wird NICHT geraten (V-041) → Lücken-Zeile. L8b-1: Wirtschaftlichkeits-Block in der geteilten `oneshot-ergebnis`-Blade an beiden Generator-Flächen (VK · W%/Ziel-Ampel · Portion · vorläufig · Lücken · Signal-Quittung) + `portion_g` im Prompt-Vertrag `vk.generator` mit Band 20–3000 g (`portionG()`) + neue Lücke `darreichung` (`ensureStandard()===null` war ein stummer Null-Preis). **L8b-2 offen:** Ziel-VK-Pill als Prompt-Constraint + Ist-gegen-Ziel im Block |
>
...
- [x] Sub-Rezept-Stubs: v1 = Stub + Flag „ausrezeptieren offen" (KEINE automatische Rekursiv-Generierung; max. Tiefe = Regelwerk §4, Entscheid für v2) — **L7b-2a**: `statistik['stubs']` nennt die NEUEN Stubs beim Namen, geteilte Blade `stub-offen` an beiden Generator-Flächen (auch mit Toggle AUS sichtbar, denn Stubs entstehen im Generier-Schritt). Der *dauerhafte* Flag ist bewusst keine neue Spalte, sondern das Signal `rezept_sub_stub_offen` (21·S1b), das genau diesen Zustand aus dem Bestand erkennt
...
- [x] Mini-Paritäts-Check am Rand (aus dem alten Audit übernommen): Hard-Stop-Inline-Anlage („GP anlegen"/„Stub anlegen") + „Meintest du?"-Disambig im FA-GeneratorModal vorhanden — sonst nachziehen. **Geprüft 2026-07-26 (L7b-2a): BEIDES fehlte** — beide Wörter standen nur als *Text*, von `offene[].shortlist` wurde nur die *Anzahl* gezeigt. **Nachgezogen in L7b-2b (2026-07-26):** `HardstopResolveService` (`stubAnlegen`/`verknuepfe`/`beschaffungAnstossen`) + Trait `HatHardstopAktionen` + geteilte Blade `hardstop-zeilen` an beiden Generator-Flächen. **Eine Abweichung von der Zeile, bewusst:** „GP anlegen" ist unter LA-First nicht baubar (der Mint hat im selben Lauf erfolglos gesucht — deshalb IST die Zeile ein Hard-Stop) → der Knopf heißt „📥 Beschaffung anstoßen" und schreibt einen Beschaffungs-Wunsch (`GpProposalService`, 07·M4), die Zeile bleibt offen. Der irreführende Alt-Text an den drei Copilot-/Revise-Flächen ist als **V-039** notiert; die fehlende MCP-Entsprechung des Ein-Zeilen-Pfads als **V-040**
...
**DoD:**
- [x] Generator-Proposal liefert/erfragt Portionsgröße verbindlich (VK-Pfad): `quantity_per_unit_g` gesetzt — ohne sie kein Auto-VK; fehlend → sichtbare Lücken-Zeile, nicht still. **L8a: die Lücken-Zeile steht** (`luecken: ['portion']`, kein stiller Null-Preis). **L8b-1: die Quelle steht** — `portion_g` ist Teil des `vk.generator`-Vertrags, `RecipeGeneratorService::portionG()` lässt nur 20–3000 g durch (unplausibel ⇒ still auf die Lücke: ein falscher Preis wäre schlimmer als kein Preis, weil er grün gemeldet würde), Basisrezept-Pfad setzt sie nie. **Ableiten aus `yield_kg / sales_unit_count` ist bewusst verworfen** — die Darreichung multipliziert mit derselben Anzahl wieder hoch (`ek_portion = EK/g × Grammatur × Anzahl`), der VK wäre der Chargenpreis → **V-041**
- [x] Accept-Kaskade: Aufschlagsklasse (Default-AK-Fallback Klasse-vor-HG) + Standard-Darreichung + Auto-VK laufen automatisch — das per KI erstellte Gericht hat nach dem One-Shot EK **und** VK **und** W%. **L8a:** Schreibweg `SalesRecipeService::updateVk` (die Darreichungs-Selbstheilung hängt dort schon dran) + `DarreichungService::ensureStandard`; Provenienz bleibt `vk_generator` statt `vk_editor`
- [x] W%-Check gegen Zielband (TeamSettings): Ausreißer → Signal (bestehendes R2.1-Signale-Muster), im Generator-Ergebnis sichtbar („Marge 22 % — unter Zielband"). **L8a:** `wareneinsatzUeberZielFuer()` aus dem Batch-Detektor herausgezogen (eine Regel, ein Dedup-Key, eine Schweregrad-Leiter) und mit der Ampel-Zahl gefüttert; **W% kommt aus der Darreichung** (`ek_portion` gegen `sales_net`), nicht aus `recipeHk` — Begründung V-041. **L8b-1: sichtbar an beiden Generator-Flächen** (Ampel-Pille W%/Ziel + Signal-Quittung "als Signal im Cockpit vermerkt")
- [x] Optional als Generator-Input: **Ziel-VK/Zielmarge als Pill** („Gericht für 8,50 € VK") → fließt als Constraint in den Prompt + wird nach der Kalkulation ehrlich gegen das Ergebnis gehalten (Brücke zu R2.4-Solver-Denke, KEIN Solver in v1). **L8b-2:** Feld `zielVk` am **VK-Generator-Modal** (nicht am Basisrezept-Modal — dort gibt es keinen VK und das Wirtschaftlichkeits-Glied liefert `null`; eine Pill ohne Folge wäre die stumme Fläche, die L8b-1 gerade beseitigt hat), Band 0,50–500 € mit **Rückfrage statt stiller Verwerfung** (Absender ist ein Mensch — anders als bei `portionG`); Transport EINMAL über `parameter.ziel_vk_eur` (Prompt-Constraint in `vk.generator`: „bau auf den Preis hin, GIB KEINEN PREIS AUS") und von dort in `RecipeOneShotService::anreichern($team, $recipe, $zielVk)`. Der Abgleich hat **keine eigene Toleranz-Schwelle**: `ziel_wareneinsatz_pct` = derselbe Bruch mit dem Ziel-VK im Nenner, beurteilt von derselben Ampel-Leiter → „Ziel erreichbar" heißt „bei diesem Preis liegt der Wareneinsatz noch im Ziel". `ziel_delta_eur` = Ist − Ziel; der Preis wird nie gedrückt. Ohne Ist-VK steht der Ziel-Wareneinsatz trotzdem (er braucht nur den EK je Portion) — dann ist er am nützlichsten. Zeile in der geteilten `oneshot-ergebnis`-Blade; MCP `recipes.GENERATE` bekommt `ziel_vk` (nur mit `vk=true`, Band-Verstoß = `VALIDATION_ERROR` statt stiller Verwerfung)
- [x] Kein Auto-Publish von Preisen: Auto-VK bleibt `auto`-Mode (überschreibbar), R2.5-Trennung interne Marge ↔ veröffentlichter VK bleibt unberührt. **L8a:** der VK wird nie geschrieben — er entsteht in `DarreichungService::recomputePreise` aus EK × Aufschlag, `price_mode` bleibt `auto` (Pest belegt beides)
- [x] Unbepreiste Zutaten (Park-GPs) schlagen sichtbar bis ins Generator-Ergebnis durch (EK partiell → VK als „vorläufig" markiert) — verzahnt mit #511-F2-Warnung. **L8a:** `vorlaeufig` aus `ek_n_ingredients_priced < ek_n_ingredients_total`; **L8b-1:** Pille + Klartext-Zeile ("Unbepreiste Zutaten im EK — der VK ist vorläufig, bis die Lead-LAs stehen")
- [x] Pest: One-Shot-VK-Roundtrip (Beschreibung → Draft mit VK + W%), Zielband-Signal-Fall, Portion-fehlt-Fall. **L8a: 7 Tests** in `RecipeOneShotWirtschaftlichkeitTest` (dazu AK-fehlt-Fall, GL-07-Überschreib-Schutz, vorläufig-Fall, Basisrezept-Gate). **L8b-1: +8** — 3 zur Portions-Quelle in `VkGeneratorTest` (Durchschreiben · 6 unplausible Werte fallen still · Basisrezept-Pfad), 4 zur Fläche in `GeneratorOneShotToggleTest` (bepreist · Lücken-Zeile · vorläufig+Signal · Basisrezept ohne Block), 1 zur neuen `darreichung`-Lücke
- [x] MCP-Lockstep: `recipes.GENERATE` (L5) liefert im Ergebnis EK/VK/W% mit. **L8a:** `recipe.sales_net` immer + `anreicherung.wirtschaftlichkeit` (sales_net/ek_pro_portion/wareneinsatz_pct/ziel_pct/ampel/luecken/vorlaeufig/signal) mit `voll_anreichern=true`; **L8b-1:** Description sagt, dass die Portion aus dem Vorschlag kommt (nicht aus dem Yield) und nennt alle drei `luecken`-Werte
