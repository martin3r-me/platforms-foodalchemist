...
>
> **→ v2-Ausbau ([Spec 20](20_Produktion_Einkauf_v2.md), ✅ 2026-07-25):** V1 „ein Auftrag je (team, production_date)" (S0/P1) ist **gekippt** — Produktionen tragen jetzt einen **Namen** (Name+Datum = Identität, mehrere/Tag). Ziel-Typen erweitert um **Basisrezept + kg** und **Foodbook-Kapitel** (Kapitel+Personen, Varianten-Wahl, eingefrorene Einzel-Ziele); Editor v2 (4-Typen-Picker), Browser/DetailPanel neu geordnet, **Stale-Marker** für veraltete Übergaben (`last_handover_at`/`handover_targets_hash`), neue MCP-Tools `production_orders.UPDATE`/`REMOVE_TARGET`/`HANDOVER`.
