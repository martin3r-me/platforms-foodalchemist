...
## 5c. ⚠️ Reihenfolge nach Weg-2-Erkenntnis (2026-07-19) — ZUERST lesen
Die Go-Live-Runde hat gezeigt: **nicht Embeddings, sondern die deterministische Terminologie-Schicht (`TerminologyService`) ist der Recall-Motor** (hybrid 66 % / 0 Leaks; embedding-only findet keinen trennbaren Floor). Konsequenz für dieses Pool-Paket: **erst die Terminologie-Lernschleife für neue Namen bauen (→ [02](02_RAG_System_FoodAlchemist.md) §9a/E7), dann Embed-Text-Tiefe (§5b), dann Pools nach Wert.** Neue Pools ohne Lernschleife vergrößern nur die Fläche, auf der neue Lieferanten-Namen ungelöst durchrutschen. Priorität: **E7 (Terminologie lernfähig) → Embed-Tiefe → kleine Pools (Lieferant/Konzept/Foodbook/Lab-Note) → LA-Pool.**

**⚠️ Korrektur 2026-07-19 zum „LA-Pool zuletzt" — der Grund ist NICHT der Ingest.** FA ist Master → **alle LAs liegen bereits in FA** (Zehntausende, Stammdaten); der Katalog-Ingest ([13](13_Preis_Katalog_Ingest_Q2.md)) ist ein *Zuträger für Frisches*, keine *Voraussetzung*. Backfill des Bestands + Observer (hält frisch, egal wie ein LA reinkommt) sind **vom Ingest entkoppelt** — jederzeit baubar. Der echte Grund für „zuletzt" ist die **Skala/Store-Grenze:** `MySqlJsonEmbeddingStore` rechnet Cosine in PHP, dokumentierte Grenze **~50k Vektoren je Partition**; heute 12.885 in der Master-Partition. **Der LA-Pool (Zehntausende) sprengt diese Grenze → er ist der Auslöser, an dem die Qdrant-/Partitionierungs-Frage zum ersten Mal real wird** (s. [02](02_RAG_System_FoodAlchemist.md) §5). Konsequenz: LA-Pool bleibt hinter den billigen kleinen Pools, **weil er die Store-Architektur stresst** — er könnte sogar vorgezogen werden, sobald die 50k-Grenze/Qdrant-Frage beantwortet ist (E4/E5 dieser Spec).
