...
>
> **→ v2-Ausbau ([Spec 20](20_Produktion_Einkauf_v2.md), ✅ 2026-07-25):** Der Einkauf ist auf ein **3-Panel-Cockpit** (Browser/Positionen/Detail) umgebaut, um **direkte Bestellungen** ohne Produktions-Bezug erweitert (`orders.CREATE`/`orders.ADD_LINE` + „＋ Artikel"-Livesearch + Bedarf-Schnellerfassung) und die **E5-Zusage** (Alternativ-Artikel je Zeile) eingelöst — plus **Preisstrategie-Switch je Schiene + „Neu quellen"** (`orders.RESOURCE`, `orders.sourcing_strategy`) und editierbare Kopf-Felder (`orders.UPDATE`).
