...
>
> **Restliche Teilschritte:** ~~S2a-2 Solver-Motor~~ ✅ 2026-07-27 (`MenuAssemblyService`, drei Verfahren: slot-unabhängig / B&B exakt / Greedy+Local-Swap) · ~~S2a-3 Erklärung~~ ✅ 2026-07-27 (`erklaere()`, bindende Vorgaben + Lockerungs-Delta als echte Wiederholungsläufe; V-061/V-062 ausgewiesen statt gefixt — beides Geschäftsentscheidung) · ~~S2b MCP + Übernahme~~ ✅ 2026-07-27 (**zwei** Tools: `assemblierung.POST` read-only Vorschau + `assemblierung.APPLY` Übernahme als `draft` über `ConceptGeneratorService::uebernehmeAssemblierung`; ein Tool mit `read_only=true`, das schreibt, wäre eine Lüge im Discovery-Index). **Damit ist R2.4 gebaut**; offen bleibt allein das **ungemessene Perf-Gate** (~1.000 Gerichte < 15 s) — auf der 26er-Fixture antwortet der Solver in 35–51 ms, das sagt über den Master nichts.
