...
### R9.1 Lieferanten-Stammblatt + Absprachen-Log · Größe L · Hängt an: nichts (FA-nativ) · ✅ GEBAUT 2026-07-19 (Engine+MCP)

**DoD:**
- [~] Lieferanten-Detailseite: Kontakte, Rollen, Status, WG-Abdeckung — **Daten+Aggregat (`SupplierService::stammblatt`) + MCP stehen; Livewire-Detail-Tabs = Folge-Slice**
...
> **v1-Note:** 4 Migrationen (Status+Konditionen auf `suppliers`, `supplier_contacts`, `supplier_agreements`, `supplier_documents`) + 3 Models + `SupplierStatus`-Enum + `SupplierService`-Erweiterung (setStatus/updateConditions/addContact/stammblatt) + `SupplierAgreementService` + `SignalTyp::VertragsfristFaellig`+Detektor + 3 MCP-Tools + `SupplierRelationTest` (3 Pest). 51er-Regression (Supplier/Stamm/Signal/LeadLa) grün. **Offen (Folge-Slice):** Livewire-Detail-Tabs im `Suppliers/Index` (Stammblatt/Absprachen/Konditionen/Lead-Steuerung) — Vorlage `Gps/DetailPanel`.

### R9.2 Lead-Lieferant-Steuerung als bediente Strecke · Größe M · Hängt an: R9.1 + R1 · ✅ GEBAUT 2026-07-19 (Engine+MCP)

**DoD:**
- [~] Lead-LA je GP **sichtbar + überschreibbar** — `LeadLaService::leadSteuerung` + `setLeadLa(reason)` + MCP `gp_lead.GET/PUT`; **UI = Folge-Slice** (Livewire-Tab)
