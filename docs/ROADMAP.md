...
**DoD:**
- [ ] Coverage-Engine: vergleicht Foodbook-/Konzept-Ist gegen Gerüst-Soll je Dimension (Menge/Diät/Preis/Saison/Dramaturgie)
- [ ] Live-Anzeige beim Befüllen: „HG vegan 0/1 ⚠ · Preisspanne DES überschritten · Herbst fehlt" — im Editor sichtbar, nicht in einem Report versteckt
- [ ] Coverage als MCP-Tool abrufbar (dieselbe Messlatte für Mensch und KI)
- [ ] Ampel-Logik: erfüllt / teilerfüllt / verletzt, mit Klick auf die Lücke → gefilterte Gericht-Suche zum Füllen
- [ ] Test: absichtlich verletztes Gerüst zeigt exakt die erwarteten Warnungen (Positiv- + Negativ-Test)

### R4.3 Phasen-Status je Foodbook/Konzept · Größe M · Hängt an: R4.1

**DoD:**
- [ ] Statusmaschine: Kontext → Struktur → Befüllung → Kalkulation → Freigabe (ersetzt draft/aktiv nicht, ergänzt sie)
- [ ] Phasen-Übergänge mit Gate-Bedingungen (z. B. Kalkulation → Freigabe nur bei Coverage ohne rote Ampeln — Override möglich, aber protokolliert)
- [ ] Phase sichtbar in Browser-Listen + filterbar
- [ ] MCP liest/setzt Phase (Freigabe bleibt menschlich — analog Rezept-Status-Regel)
...
**DoD:**
- [ ] Gericht-Baum im Concepter zeigt die Zutaten-Zeilen (read-first) mit denselben Zeilen-Aktionen wie der `IngredientEditor`
- [ ] ⇄ Tausch/♻ Ersatz erzeugen/bearbeiten eine **Slot-Variante** (konzept-lokal), Quell-Gericht bleibt unangetastet — sichtbarer „variiert"-Marker + Rücksetzen
- [ ] EK/Marge des Slots rechnet live gegen die Variante (Resolver), nicht gegen das Quell-Gericht
- [ ] Kein stiller Global-Edit: wer das Quell-Gericht ändern will, geht bewusst in den Verkauf-Editor
- [ ] MCP-fähig (Variante als Tool erzeugbar); Test: Tausch im Konzept A ändert Konzept B / Quell-Gericht nachweislich nicht
...
**DoD:**
- [ ] Input: Planungs-Gerüst (R4.1) oder Freitext-Brief → KI baut Konzept ausschließlich aus echten VK-Gerichten (keine Halluzinations-Gerichte — Slot ohne Treffer bleibt leer mit Begründung)
- [ ] Pairing-Graph prüft die Menüfolge (`cohesion`/`bridge`), Ergebnis als Kohäsions-Score + Begründung am Konzept sichtbar
- [ ] Coverage-Check (R4.2) läuft automatisch — KI-Konzept wird an derselben Messlatte gemessen wie menschliche
- [ ] Ergebnis immer `status=draft` + `created_via`-Lineage
- [ ] Blindtest: 3 echte Kunden-Briefs → Dominique bewertet die Konzepte; mindestens 2 von 3 „mit Anpassung verwendbar"
