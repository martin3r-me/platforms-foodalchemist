...
> | **L7** | 🟡 **L7a + L7b-1 gebaut**, L7b-2 offen | L7a: `RecipeOneShotService::anreichern()` + `BulkEnrichService::ZIELFELDER`/`luecken()`/`laufAnlegen()`; `voll_anreichern` an `recipes.GENERATE`. L7b-1: **V-035 entschieden** — `VkGeneratorModal` von synchron auf `GenerateRecipeJob` gehoben, Lauf-Mechanik im geteilten Trait `HatGeneratorLauf`, Toggle „⚡ Voll anreichern" (Default AN) + Ergebnis-Zeile als geteilte Blade-Komponenten an **beiden** Flächen, Job-Timeout 300→540 s bei aktiver Kaskade. L7b-2a: Kohärenz-Glied (`RecipeOneShotService::kohaerenzGlied` = `CoherenceService::judge` als GL-10-Achse 2, nur VK ab 2 Komponenten, eigenes try/catch, läuft auch ohne Anreicherungs-Lücke) + Sub-Rezept-Stub-Flag (`statistik['stubs']` mit Namen; dauerhafter Flag bleibt Signal `rezept_sub_stub_offen`), beide Zeilen an beiden Flächen. Offen (L7b-2b): Hard-Stop-**Inline-Anlage** als echte Knöpfe (heute nur Text) + „Meintest du?"-Disambig über die schon gelieferte `offene[].shortlist` (heute nur deren Anzahl) |
...
- [ ] Mini-Paritäts-Check am Rand (aus dem alten Audit übernommen): Hard-Stop-Inline-Anlage („GP anlegen"/„Stub anlegen") + „Meintest du?"-Disambig im FA-GeneratorModal vorhanden — sonst nachziehen. **Geprüft 2026-07-26 (L7b-2a): BEIDES fehlt** — „GP anlegen"/„Basisrezept anlegen" stehen an beiden Generator-Flächen nur als *Text* (`generator-modal.blade.php:103`, `vk-generator-modal.blade.php:138`), und von der mitgelieferten `offene[].shortlist` wird nur die *Anzahl* gezeigt, nicht die Kandidaten. → nachzuziehen in **L7b-2b**
