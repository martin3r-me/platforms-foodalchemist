...
> | **L8** | 🟡 **L8a gebaut 2026-07-26**, L8b offen | L8a: `RecipeOneShotService::wirtschaftlichkeitsGlied()` am Ende der Kaskade (nur VK) — Default-AK Klasse-vor-HG + `ensureStandard` + Auto-VK aus der bestehenden Darreichungs-Maschine (`price_mode='auto'`, kein Auto-Publish) + W%-Ampel/Signal über das herausgezogene `SignalDetektorService::wareneinsatzUeberZielFuer()`; `recipes.GENERATE` liefert `sales_net` + `anreicherung.wirtschaftlichkeit`. Portion wird NICHT geraten (V-041) → Lücken-Zeile. **L8b offen:** Ergebnis-Zeile an beiden Generator-Modals, `portion_g` im Prompt-Vertrag `vk.generator`, Ziel-VK-Pill |
>
...
- [x] W%-Check gegen Zielband (TeamSettings): Ausreißer → Signal (bestehendes R2.1-Signale-Muster), im Generator-Ergebnis sichtbar („Marge 22 % — unter Zielband"). **L8a:** `wareneinsatzUeberZielFuer()` aus dem Batch-Detektor herausgezogen (eine Regel, ein Dedup-Key, eine Schweregrad-Leiter) und mit der Ampel-Zahl gefüttert; **W% kommt aus der Darreichung** (`ek_portion` gegen `sales_net`), nicht aus `recipeHk` — Begründung V-041. Sichtbarkeit an der Fläche = L8b
- [ ] Optional als Generator-Input: **Ziel-VK/Zielmarge als Pill** („Gericht für 8,50 € VK") → fließt als Constraint in den Prompt + wird nach der Kalkulation ehrlich gegen das Ergebnis gehalten (Brücke zu R2.4-Solver-Denke, KEIN Solver in v1) — L8b
...
- [x] Unbepreiste Zutaten (Park-GPs) schlagen sichtbar bis ins Generator-Ergebnis durch (EK partiell → VK als „vorläufig" markiert) — verzahnt mit #511-F2-Warnung. **L8a:** `vorlaeufig` aus `ek_n_ingredients_priced < ek_n_ingredients_total`; die Anzeige-Hälfte ist L8b
- [x] Pest: One-Shot-VK-Roundtrip (Beschreibung → Draft mit VK + W%), Zielband-Signal-Fall, Portion-fehlt-Fall. **L8a: 7 Tests** in `RecipeOneShotWirtschaftlichkeitTest` (dazu AK-fehlt-Fall, GL-07-Überschreib-Schutz, vorläufig-Fall, Basisrezept-Gate)
- [x] MCP-Lockstep: `recipes.GENERATE` (L5) liefert im Ergebnis EK/VK/W% mit. **L8a:** `recipe.sales_net` immer + `anreicherung.wirtschaftlichkeit` (sales_net/ek_pro_portion/wareneinsatz_pct/ziel_pct/ampel/luecken/vorlaeufig/signal) mit `voll_anreichern=true`
