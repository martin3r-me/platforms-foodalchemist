...
> **✅ GEBAUT 2026-07-19 — Kerntemperatur-Referenz (weich, qualitäts-first).** `CulinaryReferenceService::KERNTEMPERATUREN` (32 Teilstücke) + MCP `foodalchemist.reference.GET` (read-only, kind=core_temp). **Entscheid Dominique: NICHT hart** — `target_c` = Qualitäts-Zielwert (Rind rosa 52, Geflügel-Brust 68), Sicherheit als Zeit-Temperatur-Kontext (`classic_safe_c` + `safety`-Hinweis), `is_hard_safety=true` NUR bei durchmischter Masse (Hackfleisch/Geflügel-Hack/Brät). Parasiten-Vorbehalte (Wildschwein-Trichinella, roher Fisch-TK) als Hinweis. Quelle je Zeile (BGS/Modernist Cuisine), `evidence=med`, „amtliche/lokale HACCP + Zeit-Temperatur hat Vorrang" fest drin. Keine Erfindung — etablierte Gastronomie-Werte. Pest `CulinaryReferenceTest`. Vorwärtskompatibel via `kind` für Hydrokolloid/HLB (nächster Slice).
...
> **✅ GEBAUT 2026-07-19 — Hydrokolloid-Dosier + HLB.** In `CulinaryReferenceService`: `HYDROCOLLOID_DOSAGES` (14 Agenten: Agar/Xanthan/Kappa+Iota-Carrageen/Gellan LA+HA/Alginat+Calcium/Methylcellulose/Lecithin/LBG/Pektin HM+LM — Dosier-Range % vom Ansatz + Cofaktor + thermoreversibel) + `HLB_VALUES` (8 Emulgatoren, type o_w/w_o/dual). Über dasselbe MCP-Tool `foodalchemist.reference.GET` (`kind=hydrocolloid|hlb`, Filter). Dosierung = Extraprozent (Punkt-1-Verzahnung). Quelle je Zeile (Modernist Cuisine Bd. 4 / BGS), `evidence=med`, „Herstellerangabe/Charge hat Vorrang, im Zweifel Vorversuch". Pest `CulinaryReferenceTest`. **cP-Tabelle bewusst weggelassen** (schmaler Nutzen; bei Bedarf nachziehbar). Damit sind die Referenztabellen C von #513 abgeschlossen.
