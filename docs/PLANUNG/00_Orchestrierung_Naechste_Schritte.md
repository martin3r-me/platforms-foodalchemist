...
- **16** ✅ **GEBAUT+GETESTET 2026-07-20** — WG-Lead-gescopter LA-Kandidaten-Finder + On-demand-Klassifikation (aus Use Case „Artikel unter den WG-Leads finden, wenn kein GP existiert"). Schärft Spec-07-`mintFromLa` (war `searchGlobal->items()[0]`, naiv). **Kern-Move: kein Qdrant/264k-Pool** — WG-Lead-Scope verengt, Namensmatch = Weg-2-Terminologie. S1 = Reuse (`stammSupplierIdsFor`); neu: `LaCandidateFinder` (S2) + `supplier_ids`-Filter in `baseQuery` + `wgHint` in `mintFromLa` (S3) + MCP-Hint (S5). **S4 `ClassifyLaJob`** async verdrahtet — LLM-Inhalt provider-gated (Martin). 14+29 Pest grün. Offen: S3-Decompounding (Compound-Anti-Marker), WG-Hint im KI-Rezept-Schema befüllen. Entscheide E1/E2/E4 von Dominique verarbeitet (2026-07-20).
