...
- **16** 🟢 **bau-reif (code-verifiziert 2026-07-20)** — WG-Lead-gescopter LA-Kandidaten-Finder + On-demand-Klassifikation (aus Use Case „Artikel unter den WG-Leads finden, wenn kein GP existiert"). Schärft Spec-07-`mintFromLa` (heute `searchGlobal->items()[0]`, naiv). **Kern-Move: kein Qdrant/264k-Pool** — WG-Lead-Scope (`preferred_suppliers`, 109 Einträge/15 WG) verengt 264k→wenige tausend, Namensmatch = Weg-2-Terminologie (live). Scope-Resolver S1 existiert bereits (`StammLieferantService::stammSupplierIdsFor`) → Reuse. Neu nur: `LaCandidateFinder` (S2), `supplier_ids`-whereIn in `baseQuery`, `ClassifyLaJob` (WaWi-105-Spiegel, async), WG-Hint-Param. **Blocker-frei baubar** (Finder deterministisch); Klassifikator async via Provider. Fachlich vor Bau: E1 WG-Hint-Quelle · E2 Fallback-Reihenfolge · E4 Ranking-Gewichte (Dominique). Einstieg: S1(reuse)→S2→S3, dann S4.
...
**Nachtrag 2026-07-20:** Spec **16** (WG-Lead-LA-Finder) hinzugefügt + code-verifiziert → 🟢 bau-reif; dabei **S1-Scope-Resolver als bereits existierend erkannt** (`StammLieferantService::stammSupplierIdsFor`, WG+global-NULL-Merge) → kein `PreferredSupplierService`-Neubau, Baugröße auf S–M geschrumpft. `searchGlobal`/`baseQuery` verifiziert: braucht einen `supplier_ids`-whereIn-Zweig (1 Statement). **Spec 15 (LA-Vektor-Pool) damit entkoppelt/zurückgestellt.** Neue blocker-freie Bau-Kandidaten inkl. **16·S2/S3 (LA-Finder, deterministisch)**.
