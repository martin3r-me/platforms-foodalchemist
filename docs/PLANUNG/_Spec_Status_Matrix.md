...
| **17** | Bestellwesen — mini-WaWi Bestellassistent (N-Track) | 🟢 **S0 `0d78bd2` · S1 `3daf87d` · S2 KOMPLETT (Engine+MCP `bbc73e3` + UI-Slice)** GEPUSHT 2026-07-21 · S3 offen (E1–E11) | Der von [R9 §7](14_Lieferanten_Management_R9.md) ausgeklammerte N-Track, **OHNE Bestand**. S0 = Gebinde-Bestellzeile + „N Portionen" + Einkauf-Blatt raus. S1 = `suppliers.delivery_days`/`order_cutoff_time`/`order_lead_days`. **S2 = `orders`/`order_lines` (Snapshot + source_contributions) + `OrderStatus` + `OrderService` (E1/E3/E9/E10/E11/E2 + MOQ-Ampel) + 3 MCP-Tools; `OrderServiceTest` 10/10.** Offen: S2-UI (Bestellungen-Seite + „Bedarf übernehmen"-Button) → S3 (PDF/CSV/mailto). Dev #549 |
