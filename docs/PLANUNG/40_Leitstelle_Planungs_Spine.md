...
### Phase D — Layout-Blöcke + Wahl-Gruppen (billig im Editor, Renderer-Abhängigkeit) · Status: ✅ gebaut 2026-08-20 (Branch `feat/spec40-umsetzung`) — Grouping-Optik editor-only
> **Renderer-Entscheidung (§6-Bestätigung):** Editor-Seite voll + `variant_group_id` daten-fertig in `dokumentDaten` emittiert; die „A oder B"-**Gruppierungs-Optik** im Druck bleibt bewusst **editor-only vorerst** (der Renderer folgt) — mit ehrlichem Hinweis im Edit-Panel (statt stiller Falle). **Umsetzung:** `Speisekarte/Index::layoutBlockNeu(rubrikId,type)` (header/text/spacer via addPosition) + `variantGruppeVorschlag` (`nextVariantGroupId`); Edit-Panel erweitert (editLabel für Überschrift, editVariantGroupId + Vorschlag-Button für gericht_ref/menue_ref); ✎ jetzt auch für header/text. `rubrik.blade`: „+ Ü / + Text / + ␣"-Buttons im Rubrik-Kopf, typ-abhängige Edit-Felder, Editor-only-Hinweis. `dokumentDaten` emittiert `variant_group_id`. Pest: SpeisekarteUiTest (Layout-Blöcke + Wahl-Gruppe persist + dokumentDaten-Emit) 6/6.
