...
- **Klick-Ziel R6.1:** `ConceptGeneratorService::generiereAusBrief(Team, string $brief, ?name, via='ui', bool $useConvenienceList=false)` `:70` — nimmt **rohen Freitext**, leitet das Gerüst intern via `propose('concept.brief_geruest')` neu ab. **Kein struktureller Brief-Overload** (nur `generiereAusGeruest(…, FoodAlchemistPlanningFrame,…)` ist strukturiert).
