...
| **S4b** | Tranche C · frame-gestützt | M | `konzept_preisband_verletzt` + `konzept_regel_verletzt` aus EINEM `CoverageService::coverage()`-Pass je Konzept (Preis-Kopf + `RULE_TYPES`-Befunde mit `ampel='verletzt'`) · `konzept_dramaturgie` (info) über den Anker-Graph |
