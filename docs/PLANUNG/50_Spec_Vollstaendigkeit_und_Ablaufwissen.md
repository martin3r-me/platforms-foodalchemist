...
| **7 · Container-Adapter** | ✅ `reife.GET` (alle 13 `ReifeService::KINDS`) + sechs Container-Adapter über gemeinsamem `ContainerReifeAdapter`: `concept`/`paket` · `format` · `foodbook` · `speisekarte` · `speiseplan` · `angebot`/`offer`. **Ehrliche Degradation:** Coverage nur concept/foodbook/speisekarte, DQ nur concept/foodbook, sonst `nicht_messbar` mit `warum`; kein erfundenes Tool in `wie` (Speiseplan-Kopf und Format-Edition ohne Konzept → `wie: null`); `default_pax` ist DB-Default 100 → als `nicht_messbar` deklariert statt als Lücke gemessen; `vorlaeufig` (Angebot) nur bei Preis ohne `price_calculated_at`, fehlender Preis bleibt Lücke `preis`. `naechste_schritte` nach Tool dedupliziert, Pflicht-Lücken vorn |
| **8 · E-3 bis E-6** | ✅ `ablauf.GET` (Vorgangs-Register, 10 Vorgänge) + `regelwerk.GET` (Kanon zuerst, sonst ganzes Dossier — `quelle` sagt welcher Fall) + E-5 (jedes Einstiegs-Tool zeigt in seiner Description auf `ablauf.GET(vorgang=…)`, per Wächter-Test erzwungen) + E-6 (fünf veraltete Workflow-Dossiers auf v2: Kopf-Felder `code`/`gilt_fuer_vorgang`/`required_tools`/`trigger_phrases` + Stand 2026-09-06). **Das Soll wird abgeleitet, nicht abgeschrieben:** neue `ReifeAdapter::sollAspekte()` je Adapter, die Rezept-Schrittfolgen direkt aus `BulkEnrichService::SCHRITTE`/`SCHRITTE_VK`; `ReifeSollAspekteTest` hält Deklaration und Messung synchron und verbietet Tool-Namen, die es nicht gibt. `KnowledgeContextService::regelwerkDokumentFuer()` aus dem privaten `regelwerkBlock()` herausgezogen — eine Wahrheit, zwei Aufrufer (Generator baut den Block, `regelwerk.GET` nennt das Dokument). **E-6-Befund:** die zwei Gericht-Dossiers waren keine Dubletten, sondern zwei Wege — Weg A die SaaS-KI über die Planungs-Leitstelle, Weg B der Agent in ihrer Rolle mit **derselben** Schrittfolge; zusammengeführt in `workflow.verkaufsgericht_anlegen_mcp` v6, wobei Weg B die Schritte nicht aufzählt, sondern auf `ablauf.GET → soll_aspekte` verweist. Der ältere `workflow.gericht_anlegen_mcp` ist globales Master-Wissen und per MCP weder editier- noch deaktivierbar — **offen: serverseitig stilllegen** |
| **8b · Dossier-Größe + die zwei fehlenden Vorgänge** | ✅ Ein Vorgang kann jetzt aus MEHREREN Dossiers bestehen (`doc_slugs` statt `doc_slug`; das erste ist das Leit-Dossier und liefert `required_tools`/`trigger_phrases`, fehlende Teile melden `ablauf_prosa_teilweise`) — nötig, weil **kein Dossier über 4.000 Zeichen** gehen darf (Embedding-Fenster 2.000, die Wissens-Oberfläche meldet größere als Fehler). Der Gericht-Vorgang liegt daher in vier Dossiers: Regeln für beide Wege · Weg A (Leitstelle) · Weg B (Eigenregie) · Abschluss. **Neu geschrieben:** `workflow.speisekarte_anlegen_mcp` (inkl. der stillen Falle `writing_style_id`=NULL ⇒ `speisekarte_wording.GENERATE` liefert wortlos 0) und `workflow.format_anlegen_mcp` (Format = Marke über dem Konzept, Edition = referenziertes Konzept, kein eigener Preis; die drei Einsatz-Mechaniken lebend/lebend/Einmal-Expansion für Foodbook/Angebot/Speisekarte). Ein Test hält den Deckel, mit sichtbarer Altlast-Liste statt aufgeweichter Prüfung |
| **C-7 · Angebot auf Foodbook-Niveau** | ✅ `OfferCompositionService::strukturAusGeruest()` + `offer.STRUKTUR_AUS_GERUEST` (Gerüst-Slots werden Kapitel statt eines einzigen „Menü"), `offer_chapter.REORDER`/`.MOVE`, `offer_block.REORDER`, `angebot.KUNDENTEXT_GENERATE` (war UI-only). **Drei Befunde der Spec korrigiert:** `header_source` hat am Angebot sehr wohl eine Spalte (seit 2026_08_31_000033) — es fehlte nur in `BLOCK_FELDER`, der Kommentar in `Editor.php` war falsch; der stille `text`-Rückfall ist seit A6 behoben; **neu gefunden:** die Blade-Cases hießen `header_neutral`/`header_frei`, persistiert wird `header`/`header_preis` → jeder Angebots-Header rendete als kursives „(Text)". Dazu: das Staffel-Preset ist am Angebot ausgeblendet und `price_basis` wird geprüft (Staffeln kann nur das Foodbook). Reife misst jetzt `geruest_nicht_materialisiert`; die Coverage-**Belegung** bleibt `nicht_messbar`, weil `CoverageService` den Owner-Typ `offer` weiterhin nicht kennt |
...
**Volle Suite auf stabilem Stand (2026-09-06, nach Etappe 8 + C-7): 4052 Tests, 4047 grün, 5 skipped, 0 rot** (Etappe 7: 4029/4024, Etappe 6: 4019/4014, Etappe 4: 3936/3931). `origin/main` (Strang III) am 2026-09-06 konfliktfrei eingemergt.
