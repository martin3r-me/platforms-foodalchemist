...
## Deploys 16 + 17 — 2026-09-18 19:25 / 19:50: Zutaten-Erdung greift jetzt für alle Aufrufer

- **Deploy 16, Pin `d18485ee` = PR #131:** `contextFor()` leitet Hauptzutat-Slugs aus der Beschreibung ab (`TokenEngine::leitTokens`), wenn der Aufrufer `[]` liefert und eine `zutat:grounding`-Zeile existiert; Herkunft `hauptzutaten_quelle: caller|abgeleitet`. Damit spiegelt die PREVIEW die Pipeline. Messung danach: `via: zutat_grounding` erscheint, aber **`ohne_dossier`** für passion_fruit/acerola_14, obwohl Discovery dieselben Dossiers lud.
- **Ursache (Falle 6):** `nurFuerPrompt()` schließt bei aktivem Arten-Routing (`artenRoutingAktiv`, demo wahr) alle Dokumente mit gesetzter `art` aus (`whereNull(art)`); die Zutaten-Dossiers tragen `art = fachwissen`. Für Kategorie-Discovery gewollt, für den Slug-Lookup falsch. Test war grün, weil die Fixture `art = NULL` ohne Arten-Zeile hatte.
- **Deploy 17, Pin `9e2e528f` = PR #132:** `zutatDocs()` nutzt `nurSichtbar()` + `geltendeDokumentIds()`, nicht `nurFuerPrompt()`; geerdete Slugs wandern in `$this->ausgeschlossen` (kein Doppel Grounding+Discovery). Fixture spiegelt demo (art gesetzt + Arten-Routing aktiv). Gezielt 147/147.
- **Messung nach Deploy 17 (PREVIEW „Passionsfrucht-Gelee mit Acerola, 40 Portionen"):**

| Key | Grounding (score 1) | zusätzlich via Discovery | dropped |
|---|---|---|---|
| recipe.generator (100k) | `zutat.passion_fruit--verwendung@v4`, `zutat.acerola_14--verwendung@v3` | `…--steckbrief` ×2 (unerwünscht, s. u.) | 0 |
...
## Deploy 19 — 2026-09-18 22:05: Paket I Bildstil-Dossier + KI-Produktfoto (Pin `c38cee44` = PR #138, Paul)

- `KnowledgeContextService::kanonTextFuer()` + `RecipeImageService::stilBlock()`: gebundenes Kanon-Dossier ersetzt den rotierenden `stilAnker()` und die Style-rules-Zeile (Deckel 4.000 Z., Absatzgrenze), Inhaltsregeln je Schritt bleiben im Code; Fallback = bisheriges Verhalten. Call-Log `knowledge_used`. Kanon-Editor-Dropdown kennt `recipe.product_photo`/`recipe.step_photos`.
- Knopf „KI-Produktfoto" in Rezept-Modal (Zubereitung) und VK-Modal (Anrichten & Ausgabe): `EnrichRecipeJob::nurProduktfoto`, Cache-Schlüssel je Team+Rezept als Doppel-Enqueue-Guard (Modals sind kaskadenfrei, `markBilder` ist step-gebunden), ersetzt nur das KI-Produktfoto, manuelle Fotos bleiben, Step-Editor lädt per Re-Mount-Zähler neu. Suite 4.709/4.715 (6 skipped).
- **Steuerdaten (ich, Entscheid Dominique „lege einfach zwei passende an"):** Dossiers `bildstil.produktfoto` (782 Z.) und `bildstil.schrittfoto` (822 Z.) angelegt, Kategorie regelwerk, art regel, aktiv; Kanon `prompt_key recipe.product_photo` ← `bildstil.produktfoto` pflicht, `recipe.step_photos` ← `bildstil.schrittfoto` pflicht. Feste Werte: dark slate worktop, matte white porcelain + stainless gastronorm, daylight from the left, 45°, 50mm. Look ab jetzt im Wissens-Browser änderbar, ohne Deploy.
- Aufräumen 15_GITHUB (Skript `00_INBOX/_aufraeumen_worktrees_2026-09-18.sh`, von Dominique freigegeben): 27 → 7 Worktrees, 19 → 7 Sandboxen, 103 → 10 Remote-Branches, 1 → 15 GB frei. Anlass: Pauls Suite scheiterte mit leerem Log an voller Platte.

## Deploy 20 — 2026-09-18 22:20: Spec 55 Agent in der Planung (Pin `cf28a33b` = PR #136, Oskar)

- Rückbau: `partials/agent-mount` gelöscht (26 Seiten), Sidebar-Sprachbefehl-Knopf weg, kein globales Mount mehr. Panel `@livewire('foodalchemist.voice-modal', ['planungsSessionId' => …])` in `planung/index.blade.php`, auf allen Tabs, Default eingeklappt, `wire:key` je Session. Neuer Team-Key `voice_agent_panel_planung` (Migration gelaufen, null = an), alter Key bleibt tote Spalte.
- Design-Punkte: (a) Lückencheck fragt Sektor/Anlass/Personen/Budget/Niveau vor `planung_vorschlag.POST`; (b) `komponenten_uebernahme`-Karte → Klick → Browser-Event → `Planung\Index` schreibt in Regler/Brief mit Server-Whitelist + „Agent"-Badge (verschwindet bei manueller Änderung); (c) letzter Kaskaden-Lauf + rote Schritte im Systemprompt; (d) `formats.SEARCH`/`zielgruppen.GET`/`knowledge.PREVIEW` im Katalog, Quelle-nennen-Pflicht. Katalog geschrumpft, Gedächtnis-Schlüssel Team+User+Planungs-Session mit Fallback.
- Bekannte Lücken (PR-Body/Spec 55): Lückencheck sieht nur den gesprochenen Befehl, nicht den Formularstand; `level`-Leitplanke hat keine UI-Control (vorbestehend); Konversationsmodus hängt jetzt an `voice_tts_vorlesen`. Suite 4.683 grün / 6 skipped auf dem Branch (vor Merge mit #129–#138, disjunkte Bereiche).
- Doku: `docs/PLANUNG/55_Agent_in_der_Planung.md`, Spec 54 verweist.

## Deploy 21 — 2026-09-18 23:00: Bulk-Import (Pin `6a523780` = PR #137, Lisa) und Import der Zutaten-Dossiers

- Neue MCP-Werkzeuge: `knowledge.IMPORT` (≤ 500, idempotent Slug + Inhalts-Hash, Transaktion je Eintrag, Default inaktiv, Embedding-Anstoß), `knowledge.SET_ACTIVE slugs[]` (gestaffelt über `QueueKnowledgeEmbedJob`, Chunks 50 / 20 s, kein Core-Change), `knowledge.ALIAS action=check`, `knowledge.EMBED_STATUS` (über Store-Contract, **demo-Embeddings liegen in Qdrant**, nie Core-MySQL-Tabelle zählen). Retrieval-Schutz: `category=zutat` aus der Arten-Discovery ausgeschlossen; Regex `--<aspekt>(-…)?` für `-aroma-2`; Zähler 8 = Anker, nicht Dokumente (Bonus-Fund). Suite 4.747/4.753.
- **Entscheide (Dominique, für die Import-Session):** keine Aliasse; Deckel 3.900 bleibt (Embedding = Titel + content_md, gemessen 3.904 max); Sonderzeichen-Regel in `_KONVENTION.md` gestrichen (kein Validator im Schreibpfad, von mir angepasst inkl. Changelog); `content_md` = ganze Datei inkl. YAML; Tarocco ohne Anker auslassen.
- **Import durch mich (Dominique: „den Import übernimmst du"):** 24 Blöcke à 500 aus `07_WISSEN/_import/split2/md_import` + `manifest.json`, serverseitig über `KnowledgeService::import()` (identisch zum MCP-Tool; 30 MB gehen nicht durch den Chat). Ergebnis: **11.464 angelegt, 2 aktualisiert, 0 abgelehnt**, ~9 s je Block. Aktivierung blockweise über `setActiveBatch()`; Block 1 sofort, Blöcke 2–24 per Server-Loop, nächster Block erst bei Queue < 150 Jobs (2 default-Worker sind die natürliche Drossel; Queue-Treiber database).
- **Live-Probe nach Block 1:** „Aprikosen-Kompott mit Ahornsirup und Anis" → `zutat.maple_syrup--verwendung-kombination` + `--verwendung-komponenten` via Grounding (sent 2.564 + 3.352); Tomatensuppe-Steps: **kein** zutat.*-Dossier über Discovery (Ausschluss wirkt). Lücke: `aprikosen` → `ohne_anker` (Plural vs. Label „Aprikose") → Lisa `fix/anker-exakt-plural` (Singular-Fallback nur bei Eindeutigkeit, Komposita über Decompounder).
- Tinker-Falle: `php artisan tinker <datei.php>` führt die Datei aus und bleibt dann in der REPL hängen (Prozess 10 Min „busy") → `--execute="require …"` mit `< /dev/null` nutzen. `pkill -f` mit Muster, das die eigene ssh-Kommandozeile trifft, killt die Sitzung (Exit 255).

## Deploy 22 — 2026-09-19 00:05: Paket J Grundprodukt-Anker (Pin `5588416c` = PR #143, Paul) + Zuordnungs-Import

- **Befund (Dominique im GP-Editor):** Rezepte haben „Anker verknüpfen", Grundprodukte nicht; UND: mit dem Wechsel auf das neue Anker-Vokabular (2.628, englische Slugs) wurden die alten Anker (767, deutsch) samt aller 9.403 GP-Zuordnungen auf 7.550 GPs gedroppt (`PairingDropLegacyAnchorsCommand`), ohne Brücke → `foodalchemist_gp_anchor_mappings` auf demo leer, Pairing-Lookup „welcher GP trägt Aroma X" überall leer, Zutaten-Dossiers von GPs aus unerreichbar.
- **Paket J:** GP-Modal Sensorik & Pairing „Aroma-Anker" (Liste, Rolle je Chip, Suche slug/display_de/display_en, sofort gespeichert), GP-Detail-Chips mit „weitere GPs mit diesem Anker" (kein Graph-Umbau), MCP `gp_anchors.PUT/IMPORT/GET`, `setGpAnker()` um role/source erweitert, `removeGpAnker()`. Grundsatz korrigiert: **mehrere Anker je GP** (Altdaten: Ratatouille → eggplant/tomato/zucchini; `CAP_GP=3`), nicht einer. Suite 4.735/4.741.
- **Zuordnungs-Import (ich):** Brücke alt→neu über display_de (328 von 767 alten Ankern eindeutig) + exakter Namensabgleich (`main_ingredient_slug`/display) → **4.207 Zuordnungen auf 3.695 GPs gesetzt** (Quellen `bridge_alt_neu` 3.470, `bridge_exact` 506, `exact_name` 231), 433 GPs mit mehreren Ankern. Serverseitig über `PairingService::setGpAnker()`. Falle: `source` ist varchar(16) → 506 Fehler mit 20-Zeichen-Label, korrigiert → Paul `fix/gp-anker-source-laenge`. Review-Listen im Vault `00_INBOX/gp_anker_review.csv` (210 Zeilen, alte Anker mehrdeutig), `gp_anker_offen.csv` (4.182 GPs, v. a. Convenience/Mischungen), `gp_anker_vorschlag.csv` (Heuristik erstes Wort/Singular, nur zum Gegenlesen, NICHT gesetzt).
- Import-Stand Zutaten-Dossiers: Aktivierung Block 19/24, alle mit Embedding, 0 failed jobs.

## Import abgeschlossen — 2026-09-19 00:20

- Alle 24 Blöcke aktiviert (Loop 22:13 UTC), Queue 0, **0 failed jobs**, 11.975 aktive `zutat`-Dossiers, Stichproben `zutat.a*` 533/533 und `zutat.t*` 300/300 mit Embedding (Qdrant, per Store-Contract gezählt). Die 8 alten 6er-Schema-Dossiers sind nicht mehr im Bestand (Gesamt 11.975 = 11.966 + 9 Legacy im 4er-Schema).
- **Ende-zu-Ende-Probe** `recipe.generator` „Zucchini-Gratin mit Tomate, Thymian und Parmesan, 40 Portionen": 4 Anker geerdet → 8 Dossiers (`zucchini_2629--verwendung-kombination/-komponenten`, `tomato--verwendung-kombination/-komponenten`, `thyme--verwendung`, `parmesan_cheese--verwendung-baukasten/-komponenten/-verwertung`), alle score 1, sent > 0, Teilstücke je Aspekt gemeinsam; kein zutat.* über Discovery; Budget 100k zu 99.776 gefüllt, dropped 5.255 (Nährwertberechnung, Sauce-Tomate). Hinweis: bei 5+ Zutaten wird das 100k-Budget knapp → Kandidat 120k nach Erfahrung.
- GP↔Anker: 4.707 Zuordnungen auf 4.195 GPs live (Deploy 22). Offen: Paul `fix/gp-anker-source-laenge` (Suite läuft), Lisa #142 Plural-Fallback (Suite läuft).

## Deploy 23 — 2026-09-19 00:40: Anker-Auflösung Singular-Fallback (Pin `8878b504` = PR #142, Lisa)
...
## Paket K — Planungs-Cockpit-Optik (2026-09-19, Dominique 01:10: „UI in jedem Tab der Planung anpassen, cooles, modern wirkendes Erlebnis")

**Entscheide Dominique (AskUserQuestion):** nur Optik, keine Feld-/Ablauf-/Logikänderung · Look „Cockpit, dunkel, präzise" · alles sichtbar, aber gruppiert · direkt als Pilot im Code (Basisrezept-Tab), dann Ausrollen. Nachtrag 09:45: Design-Umbau und Agent-Umzug (Spec 55 Nachtrag) gehören ZUSAMMEN, pro Tab genau EINE Diktierfunktion; neuer Agent Nora (cooking-jarvis-c6, Opus) für den Rollout.

- **Deploy 25 — 09:15, Pin `d66c440c` = PR #149 (Peter), Pilot Basisrezept-Tab:** sechs Karten (Eingabe · Küche · Anspruch · Anreicherung · Constraints · Ziel) mit Kurz-Zusammenfassung im Kopf, Go-Leiste sticky, `data-planung-agent-slot` in der Eingabe-Karte; Pilot als `@if($scope==='rezept')`-Zweig in leitplanken.blade.php (Vorgabe: beim Rollout zu EINER Struktur verschmelzen). Suite 4.774/4.780. Screenshots aus der Sandbox nicht möglich (geteilte Dev-DB hängt an einer Fremdmodul-Migration; Peter hat richtig nichts angefasst), Abnahme live.
- **Feedback Dominique 09:20 (Screenshots):** viel zu breit (Karten über 2.500 px für wenig Inhalt), Kartenüberschriften wie Fußnoten (Uppercase-Mikro-Label), sticky Go-Leiste überdeckt die Anreicherung-Karte. → Peter `fix/planung-cockpit-breite` (a98b60e3): 2-spaltiges Raster ab xl, Lesebreite in den Karten (Inputs max-w-sm/md), Container max-w-7xl, Überschriften auf Button-Niveau mit Icon + Status-Pill, Padding unter der Go-Leiste, Karten kompakter. Zusätzlicher Befund (Nora): Fließtext-Absätze auf `$label` = Versalien → normale Schreibung mit Hilfetext-Token, `$label` nur für Mikro-Labels.
- **Rollout (Nora, PR #150, off a98b60e3, 168/168 gezielt):** alle 10 Tabs — Gericht/Concept (Ziel-Karte scope-abhängig, Struktur-Karte Concept, leitplanken-Zweig verschmolzen), Format/Foodbook/Speisekarte/Speiseplan/Angebot (Auswahl | Aus Brief planen, Status-Pill, schlichtes Diktat bleibt + leerer Agent-Slot), Composer (5 Karten, Token-Cleanup, 2-Spalten-Grid bleibt), Import (3-Schritt-Karten + Schritt-Pille, sticky nur in Schritt 2), Worker (Leerzustand als Karte, Banner auf Canvas bleibt). ~45 Farbreste (`text-slate-100…500`, `ring-offset-slate-900`, invertierte Hover) auf Theme-Tokens; drei geteilte Bausteine (worker-praesenz, schnellstart-chips, Composer-Seed-Hinweis) mitgezogen; `schnellstart-speichern` als Karte; Icons je Karte (Format → rectangle-stack, sparkles nur Anreicherung). Hex `#0b1120` am Composer-Netz bleibt (Absicht). Hinweis: Kartenflächen sind im Code `bg-white/90`, das Plattform-Theme rendert sie im Dark Mode dunkel — die Farbreste folgten dem Theme nicht.
- **Spec 55 Nachtrag „Agent am Brief" (Oskar, `feat/agent-am-brief` off a98b60e3):** Panel raus aus dem Board-Kopf, rein in den Agent-Slot der Eingabe-Karte je Creation-Scope; alter Knopf „Briefing diktieren" in den drei Creation-Tabs weg (`$mitRecorder`-Schalter in diktat.blade.php, Ausgabe-Tabs behalten es), Panel-Mikro füllt das Beschreibungsfeld; Formularstand (Brief + Regler je Scope) als Kontext; Pflicht-Leitplanken je Scope; Rückfragen mit Chips aus dem Server-Vokabular, Antwort per Klick/Sprache setzt den Regler direkt (Antwort auf konkrete Frage = Bestätigung), Rückgängig; Übernehmen-Karte nur für unaufgeforderte Vorschläge; „level" hat doch eine UI-Control (Pill-Loop), Doku korrigiert. Offen: Format-Tab ohne Regler-Satz → Nachtrag 2.
- **Merge-Reihenfolge:** Peter Breite-Fix (Deploy 26) → Oskar Agent am Brief → Nora Rollout; danach Abnahme Dominique.

## Deploys 26–28 — 2026-09-19 11:05 / 11:40 / 12:05: Paket K komplett

- **Deploy 26, Pin `d4b9e50d` = PR #152 (Peter):** Breite/Kopfzeilen/Overlap-Fix + Versalien-Korrektur im Pilot; `modal-section` mit `icon`-Prop. Suite 4.774/4.780.
- **Deploy 27, Pin `4bc54a7b` = PR #153 (Oskar, Spec 55 Nachtrag „Agent am Brief"):** Panel im Agent-Slot der Eingabe-Karte je Creation-Scope, Board-Panel weg, alter Diktat-Knopf in den drei Creation-Tabs weg (`$mitRecorder=false`), Formularstand-Kontext, Pflicht-Leitplanken je Scope, Rückfrage-Chips aus Server-Vokabular (`Index::SEKTOR_/OCCASION_/SERVICEFORM_OPTIONEN`), Direkt-Set per Chip/Sprache mit Server-Validierung, Rückgängig. Suite 4.794/4.800.
- **Deploy 28, Pin `9da2fe66` = PR #150 (Nora):** Rollout auf alle 10 Tabs, leitplanken-Zweig zu EINER Struktur verschmolzen, ~45 Farbreste auf Theme-Tokens, drei geteilte Bausteine, Icons je Karte, `schnellstart-speichern` als Karte. Beim Rebase auf #153 stille Doppelquelle abgefangen (Vokabulare als lokale Arrays UND Index-Konstanten → nur noch Konstanten). Zwei Badge-Lücken geschlossen (`ziel_portion_g`, `level` im Basisrezept-Tab), mit Oskar abgestimmt. Suite 4.794/4.800, Nachlauf gezielt 169/169.
- **Stand:** Planungs-Leitstelle in Cockpit-Optik auf allen Tabs; pro Tab genau eine Diktierfunktion (Agent-Panel in Basisrezept/Gericht/Concept, schlichtes Diktat + leerer Agent-Slot in Format/Foodbook/Speisekarte/Speiseplan/Angebot). **Abnahme Dominique offen.** Nachtrag 2 (Agent-Panel auf Ausgabe-Tabs) braucht Regler-Sätze je Scope.
