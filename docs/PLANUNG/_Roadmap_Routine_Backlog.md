...
| 3 | 2026-08-14 | **`view:cache` in der Sandbox meldet einen Fremd-Fehler** (`platform-crm/src/CrmServiceProvider.php:98`, `Facade::__callStatic("hasTable")` ohne DB). Kompilation läuft trotzdem durch (665 Views, 0 Lint-Fehler) — aber der Blade-Lint-Guard liest sich dadurch bei jedem Lauf wie ein Rotlicht. Fremdmodul, wird von dieser Routine nicht angefasst. | `platform/modules/platform-crm` (Fremdmodul → Martin) | neu |
