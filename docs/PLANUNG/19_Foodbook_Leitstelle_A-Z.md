...
- [ ] E1.5 Dedup kapitelweit in `uebernehmeVorschlag` (Konzept-Slots ∪ recipe_ref-Blöcke); Pest

### E2 — Kapitel-Baum + Coverage-Tiefe (M)
- [ ] E2.1 Unterkapitel-UI (anlegen unter Kapitel, verschieben; Service `addKapitel(parentId)`/`moveKapitel` existieren)
- [ ] E2.2 Coverage Nachfahren-Scope: `istFoodbook`-Rollup (Kinder-Aggregation) + `slotScope` = Kapitel+Nachfahren; Model-Kommentar `FoodAlchemistFoodbook.php:32` fixen; Pest Eltern-Ziel/Enkel-Gericht → erfüllt
