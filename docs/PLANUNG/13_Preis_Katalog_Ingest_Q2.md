...
**Lieferanten-Seite: DUALER WEG (Dominique 2026-07-18) — zwei Kanäle, EINE Eingangs-Verarbeitung:**
- **Kanal A — Daten-Hub-Schnittstelle (Supplier-Management):** es wird eine Schnittstelle zu einem externen Daten-Hub geben, der Lieferanten-Preislisten liefert. *(Details offen: welcher Hub, Format, Zeithorizont — klären, sobald konkret.)*
- **Kanal B — eigener Preislisten-/Artikel-Import (Datei):** bleibt IMMER als zweiter Weg — und zwar mit **vollem Artikel-Stammdatensatz**, nicht nur Preisen: **Nährwerte, Allergene, Zusatzstoffe, Lieferbedingungen**. Zieltabellen existieren (`supplier_items`/`prices` + `item_nutritionals`/`item_allergens`/`item_declarations`); Lieferbedingungen (Mindestbestellwert, Frei-Haus, Zahlungsziel …) → verzahnt mit den R9-Konditionsfeldern ([14](14_Lieferanten_Management_R9.md)).
- **Prinzip:** beide Kanäle münden in **dieselbe** Import-/Validierungs-Strecke (Mapping, Dedup, Preis-Alarm-Trigger) — der Hub ist nur eine zweite Quelle, kein zweites System.

**Verkaufs-Seite (Ist):** Bankett-/Verkaufsdaten (CSV/Excel, z.B. Bankettprofi) → speist R2.3. *(Format-Spec = das offene Gate, s. [12](12_Wirtschaftlichkeits_Intelligenz_R2-Rest.md).)*
...
- [ ] **Kanal B — Voll-Artikel-Import:** eigenes Preislisten-Format (dokumentierte Vorlage) deckt neben Preis auch **Nährwerte, Allergene, Zusatzstoffe, Lieferbedingungen** ab → schreibt `item_nutritionals`/`item_allergens`/`item_declarations` + R9-Konditionsfelder; Allergen-/Nährwert-Änderungen propagieren über die bestehende Live-Auflösung (GL-01) in GPs/Rezepte.
- [ ] **Kanal A — Hub-Schnittstelle:** Anbindung an den Supplier-Management-Daten-Hub, sobald spezifiziert — auf DIESELBE Import-/Validierungs-Strecke wie Kanal B (kein Parallel-Pfad).
