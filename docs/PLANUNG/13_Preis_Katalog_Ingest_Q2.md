...
- [ ] **Kanal B Voll-Artikel-Import:** dokumentierte Vorlage deckt Preis + Nährwerte + Allergene + Zusatzstoffe + Lieferbedingungen → schreibt `item_nutritionals`/`item_allergens`/`item_declarations` + Konditions-Spalten; Allergen-/Nährwert-Änderungen propagieren. **Zu 4/5 erfüllt (S1c 2026-07-27):** Preis + die drei Detail-Tabellen stehen, offen bleiben die Konditions-Spalten (S2). **Korrektur zur Zeile:** die Propagierung ist *keine* Live-Auflösung — Rezept-Allergene/-Zusatzstoffe/-Nährwerte sind aggregierte Spalten (`RecipeRecomputeService::recomputePipeline`), darum zählt ein geänderter Detail-Wert seit S1c als „bewegter Artikel" und zieht dieselbe E4-Kette wie ein geänderter Preis.
