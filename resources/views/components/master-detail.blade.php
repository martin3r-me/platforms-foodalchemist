...
<div {{ $attributes->merge(['class' => "flex gap-4 {$height}"]) }}
    ...
    @isset($panel)
        <aside x-show="panelOpen" x-cloak
            ...
            <div class="shrink-0 flex items-center justify-end px-2 pt-2">
                <button type="button" @click="panelOpen = false"
                        class="p-1 rounded-md text-gray-400 hover:text-violet-600 dark:hover:text-violet-400 hover:bg-black/5 dark:hover:bg-white/10 transition-colors duration-150"
        ...
        <button type="button" x-show="!panelOpen" x-cloak @click="panelOpen = true"
                class="w-8 shrink-0 flex items-start justify-center pt-2 {{ $card }} text-gray-400 hover:text-violet-600 dark:hover:text-violet-400 transition-colors duration-150"
