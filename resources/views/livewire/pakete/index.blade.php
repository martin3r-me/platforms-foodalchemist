...
<x-ui-page>
    ...
    <x-slot name="activity">
        <x-ui-page-sidebar title="Paket" width="w-96" :maxWidth="640" storeKey="activityOpen" side="right">
                    ...
                    <div class="space-y-2 pt-2 border-t border-black/5 dark:border-white/10">
                        ...
                        @if($form['price_mode'] === 'auto')
                            <div class="flex items-center justify-between">
                                <span class="text-[11px] text-gray-500 dark:text-gray-400">Preis wird aus den Gerichten berechnet.</span>
                            ...
                            @forelse($selected->dishes as $g)
                                <div wire:key="bg-{{ $g->id }}" class="flex items-center gap-2 px-2 py-1 rounded-lg bg-black/[0.03] dark:bg-white/5 text-xs">
                                    <span class="flex flex-col -my-0.5 shrink-0">
                                        <button type="button" wire:click="gerichtHoch({{ $g->id }})" class="text-gray-500 dark:text-gray-400 hover:text-violet-500 leading-none" title="hoch">▲</button>
                                        <button type="button" wire:click="gerichtRunter({{ $g->id }})" class="text-gray-500 dark:text-gray-400 hover:text-violet-500 leading-none" title="runter">▼</button>
                                    </span>
                                    ...
                                    <span class="text-gray-500 dark:text-gray-400 tabular-nums shrink-0 w-14 text-right">{{ $g->dish?->sales_net !== null ? number_format((float) $g->dish->sales_net, 2, ',', '.') . ' €' : '' }}</span>
                                    <button type="button" wire:click="gerichtRaus({{ $g->sales_recipe_id }})" class="text-gray-500 dark:text-gray-400 hover:text-red-500 shrink-0" title="Entfernen">✕</button>
                                </div>
                            @empty
                                <p class="text-[11px] text-gray-500 dark:text-gray-400">Noch keine Gerichte. Unten suchen und hinzufügen.</p>
                            @endforelse
                        </div>
            ...
            @else
                <div class="p-6 text-center text-sm text-gray-500 dark:text-gray-400">Paket auswählen oder neu anlegen.</div>
            @endif
        </x-ui-page-sidebar>
    </x-slot>
    ...
    <x-ui-page-container padding="px-6 pb-6" spacing="space-y-4">
        <div class="flex items-center justify-between pt-1">
            ...
            <p class="text-[11px] text-gray-500 dark:text-gray-400">Paket = bepreistes Bündel mehrerer Gerichte für eine Rolle. Im Concept tauschbar gegen Pakete derselben Rolle.</p>
        </div>
                    ...
                    <tbody>
                        @forelse($pakete as $b)
                            <tr wire:key="b-{{ $b->id }}" wire:click="waehle({{ $b->id }})"
                                ...
                                <td class="{{ $td }} text-gray-600 dark:text-gray-400 whitespace-nowrap">{{ $b->role ?? '—' }}</td>
                                <td class="{{ $td }} text-gray-600 dark:text-gray-400 whitespace-nowrap">{{ $b->level ?? '—' }}</td>
                                <td class="{{ $td }} text-gray-600 dark:text-gray-400 text-right tabular-nums">{{ $b->gerichte_count }}</td>
                                ...
                                <td class="{{ $td }} text-gray-600 dark:text-gray-400 text-right tabular-nums">{{ $b->food_cost_percent !== null ? number_format((float) $b->food_cost_percent, 1, ',', '.') . ' %' : '—' }}</td>
                                <td class="{{ $td }}"><span class="{{ $pill }} {{ $b->price_mode === 'auto' ? $variantPill['info'] : $variantPill['secondary'] }}">{{ $b->price_mode }}</span></td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="px-5 py-10 text-center text-gray-500 dark:text-gray-400">Noch keine Pakete. Oben „+ Neuer Paket".</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
