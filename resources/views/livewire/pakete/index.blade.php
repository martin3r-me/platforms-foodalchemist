...
<x-ui-page>
        ...
        <x-ui-page-sidebar title="Rollen" width="w-72">
            <div class="p-3 space-y-2">
                ...
                <button type="button" wire:click="$set('rolleFilter', '')"
                        class="w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-xs {{ $rolleFilter === '' ? 'bg-gradient-to-r from-violet-500/10 to-indigo-500/10 text-violet-700' : 'text-gray-700 hover:bg-black/[0.03]' }}">
                ...
                <div class="space-y-0.5 -mx-1">
                    @foreach($rollen as $role)
                        <button type="button" wire:key="role-{{ $loop->index }}" wire:click="$set('rolleFilter', @js($role))"
                                class="w-full flex items-center px-2 py-1 rounded-lg text-xs {{ $rolleFilter === $role ? 'bg-gradient-to-r from-violet-500/10 to-indigo-500/10 text-violet-700' : 'text-gray-600 hover:bg-black/[0.03]' }}">
                ...
                <div class="p-4 space-y-4" wire:key="edit-{{ $selected->id }}">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button type="button" wire:click="loeschen({{ $selected->id }})"
                                wire:confirm="Paket löschen?" class="{{ $btnGhost }} text-red-600">Löschen</button>
                    </div>
                    ...
                    <div class="space-y-2 pt-2 border-t border-black/5">
                        ...
                        <div class="space-y-1">
                            @forelse($selected->dishes as $g)
                                <div wire:key="bg-{{ $g->id }}" class="flex items-center gap-2 px-2 py-1 rounded-lg bg-black/[0.03] text-xs">
                                    <span class="flex flex-col -my-0.5 shrink-0">
                                        <button type="button" wire:click="gerichtHoch({{ $g->id }})" class="text-gray-500 hover:text-violet-500 leading-none" title="hoch">▲</button>
                                        <button type="button" wire:click="gerichtRunter({{ $g->id }})" class="text-gray-500 hover:text-violet-500 leading-none" title="runter">▼</button>
                                    </span>
                                    ...
                                    <span class="text-gray-500 tabular-nums shrink-0 w-14 text-right">{{ $g->dish?->sales_net !== null ? number_format((float) $g->dish->sales_net, 2, ',', '.') . ' €' : '' }}</span>
                                    <button type="button" wire:click="gerichtRaus({{ $g->sales_recipe_id }})" class="text-gray-500 hover:text-red-500 shrink-0" title="Entfernen">✕</button>
                                </div>
                            @empty
                                <p class="text-[11px] text-gray-500">Noch keine Gerichte. Unten suchen und hinzufügen.</p>
                            @endforelse
                        </div>
    ...
    <x-ui-page-container padding="px-6 pb-6" spacing="space-y-4">
        <div class="flex items-center justify-between pt-1">
            ...
            <p class="text-[11px] text-gray-500">Paket = bepreistes Bündel mehrerer Gerichte für eine Rolle. Im Concept tauschbar gegen Pakete derselben Rolle.</p>
        </div>
        <div class="relative overflow-hidden {{ $card }}">
            ...
            <div class="px-5 py-3 border-t border-black/5">{{ $pakete->links() }}</div>
        </div>
    </x-ui-page-container>
</x-ui-page>
