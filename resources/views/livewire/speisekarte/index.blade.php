    ...
    <x-ui-page-container padding="px-6 pb-6" spacing="space-y-4">
        ...
        @else
            ...
            <div class="{{ $card }} p-4 mt-4 space-y-3" data-sk-praesentation>
                ...
                @if($presentationHinweis)<div class="rounded-lg bg-emerald-500/10 border border-emerald-500/25 text-emerald-300 text-xs px-3 py-2" data-sk-praes-hinweis>{{ $presentationHinweis }}</div>@endif
                @if($presentationFehler)<div class="rounded-lg bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs px-3 py-2" data-sk-praes-fehler>{{ $presentationFehler }}</div>@endif
                ...
                @if($presentationLink)
                    <div class="rounded-lg bg-white/[0.04] border border-white/10 p-3 text-xs" x-data>
                        <div class="flex items-center gap-2">
                            <div class="flex-1 rounded px-2 py-1 font-mono text-[11px] break-all select-all bg-black/30 border border-white/10 text-gray-200" data-sk-praes-link>{{ $presentationLink }}</div>
            ...
            <div class="relative overflow-hidden {{ $card }} p-4" wire:key="sk-body-{{ $karte->id }}"
                ...
                <div class="flex gap-4 items-start" data-sk-2col>
                    ...
                    {{-- Persistenter Katalog (geteilter Baustein): Gericht · Menü · Format. Gericht/Menü fügen
                         in die Ziel-Rubrik (per „+" an einer Rubrik gewählt); Format wird zur eigenen Rubrik
                    ...
                    <x-foodalchemist::katalog-picker marker="sk" switch="katalogModus" :modes="[
                        ...
                        @if($pickerModus === 'format')
                            ...
                            @error('formatRubrik')<p class="text-[11px] text-rose-500 px-1 mb-1 shrink-0">{{ $message }}</p>@enderror
                            <p class="text-[10px] text-gray-500 mb-1 shrink-0">Bucht ein Format als eigene Rubrik (Editionen live).</p>
