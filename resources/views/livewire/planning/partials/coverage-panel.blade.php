...
<div class="relative overflow-hidden {{ $card }}" data-coverage-panel data-coverage-gesamt="{{ $coverage['ampel_gesamt'] }}">
    ...
    <div class="px-4 py-3 space-y-2" x-data="{ covAuf: {{ ($z['verletzt'] ?? 0) > 0 ? 'true' : 'false' }} }">
        <button type="button" @click="covAuf = !covAuf" class="w-full flex items-center gap-2 text-left">
            ...
            <span class="text-[11px] uppercase tracking-wider text-gray-500">Soll/Ist-Coverage (Planungs-Gerüst)</span>
            <span class="ml-auto flex items-center gap-2 text-[11px] tabular-nums">
                @if(($z['erfuellt'] ?? 0) > 0)<span class="text-emerald-600">● {{ $z['erfuellt'] }}</span>@endif
                @if(($z['teilerfuellt'] ?? 0) > 0)<span class="text-amber-600">● {{ $z['teilerfuellt'] }}</span>@endif
                @if(($z['verletzt'] ?? 0) > 0)<span class="text-rose-600">● {{ $z['verletzt'] }}</span>@endif
                <span class="text-gray-500" x-text="covAuf ? '▾' : '▸'"></span>
            </span>
        </button>
        ...
        <div x-show="covAuf" x-cloak class="space-y-1">
            @forelse($coverage['befunde'] as $i => $b)
                <div wire:key="cov-{{ $i }}" class="flex items-start gap-2 rounded-lg px-2 py-1 text-[11px] {{ $b['ampel'] === 'verletzt' ? 'bg-rose-500/5' : '' }}" data-coverage-befund="{{ $b['dimension'] }}">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <span class="text-gray-600"> — Soll: {{ $b['soll'] }} · Ist: {{ $b['ist'] }}</span>
                        @if($b['hinweis'])<div class="text-gray-500">{{ $b['hinweis'] }}</div>@endif
                    </div>
                    @if($b['fill_filter'] !== null && $coverageFillAction !== null)
                        <button type="button" wire:click="{{ $coverageFillAction }}('{{ $b['fill_filter']['diet_form'] ?? '' }}')"
                                class="{{ $btnGhostXs }} text-violet-600 shrink-0" title="Gefilterte Gericht-Suche zum Füllen" data-coverage-fuellen>
                            → füllen
                        </button>
            ...
            @empty
                <p class="text-[11px] text-gray-500">Gerüst vorhanden, aber ohne prüfbare Vorgaben — im Planungs-Gerüst Soll-Werte setzen.</p>
            @endforelse
        </div>
    </div>
</div>
