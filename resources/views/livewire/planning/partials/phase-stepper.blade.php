...
<div class="space-y-1.5" data-phase-stepper data-phase-aktuell="{{ $phaseAktuell }}">
                    ...
                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] border transition-colors
                        {{ $key === $phaseAktuell
                            ...
                            : ($i < (int) $aktuellerIndex
                                ...
                                : 'bg-black/[0.03] border-transparent text-gray-600 hover:text-gray-700') }}"
    ...
    @if($phaseFehler)
        <div class="rounded-lg bg-rose-500/10 border border-rose-500/30 px-3 py-1.5 text-[11px] text-rose-700" data-phase-fehler>{{ $phaseFehler }}</div>
    @endif
    @if($phaseOverrideOffen)
        <div class="flex items-center gap-2" data-phase-override>
            ...
            <button type="button" wire:click="phaseSetzen('freigabe')" class="{{ $btnGhostXs }} text-rose-600">Trotzdem freigeben</button>
        </div>
    @endif
</div>
