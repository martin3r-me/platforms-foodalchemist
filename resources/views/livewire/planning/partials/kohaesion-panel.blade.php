...
<div class="relative overflow-hidden {{ $card }}" data-kohaesion-panel>
    ...
    <div class="px-4 py-3 space-y-2">
        <div class="flex items-center gap-2">
            <span class="text-[11px] uppercase tracking-wider text-gray-500 dark:text-gray-400">Kohäsion der Menüfolge (Pairing-Graph)</span>
        ...
        @if($menueKohaesion !== null)
            @if($menueKohaesion['zu_wenig'] ?? false)
                <p class="text-[11px] text-gray-500 dark:text-gray-400">Mindestens 2 Gerichte nötig — erst den Aufbau befüllen.</p>
            @else
                ...
                <div class="flex items-baseline gap-3">
                    ...
                    <span class="text-[11px] text-gray-500 dark:text-gray-400">Score · {{ $menueKohaesion['rated_pairs'] }}/{{ $menueKohaesion['total_pairs'] }} Gericht-Paare bewertet ({{ $menueKohaesion['coverage_pct'] }} % Graph-Abdeckung)</span>
                </div>
                @if($menueKohaesion['weakest_pair'] !== null)
                    <p class="text-[11px] text-gray-600 dark:text-gray-400">Schwächstes Paar: <span class="font-medium">{{ $menueKohaesion['weakest_pair']['a'] }}</span> ↔ <span class="font-medium">{{ $menueKohaesion['weakest_pair']['b'] }}</span> ({{ $menueKohaesion['weakest_pair']['score'] }}, {{ $menueKohaesion['weakest_pair']['type'] }})</p>
                @endif
                @if(($menueKohaesion['komponenten'] ?? []) !== [])
                    <div class="flex flex-wrap gap-1">
                        @foreach($menueKohaesion['komponenten'] as $k)
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] bg-black/[0.03] dark:bg-white/5 text-gray-600 dark:text-gray-400" title="{{ $k['rated_links'] }} bewertete Verbindungen">
                ...
                @if(($menueKohaesion['unrated_pairs'] ?? []) !== [])
                    <p class="text-[10px] text-gray-500 dark:text-gray-400">{{ count($menueKohaesion['unrated_pairs']) }} Paar(e) ohne Graph-Daten — ehrlich unbewertet, nicht schlecht.</p>
                @endif
            @endif
        @endif
    </div>
