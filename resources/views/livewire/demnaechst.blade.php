...
<x-ui-page>
    ...
    <x-ui-page-container padding="px-6 pb-6" spacing="space-y-4">
        <div>
            ...
            <p class="text-[11px] text-gray-400 mt-0.5">Entschieden 2026-06-12: erst die Basis fertig (M9), dann Foodbook (M10), dann Brainstorming je Domäne. Der Chat-Assistent wurde verworfen.</p>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-3" data-demnaechst-liste>
            @foreach($domaenen as $d)
                <div class="relative overflow-hidden {{ $card }} px-4 py-3" wire:key="dom-{{ $loop->index }}">
                    ...
                    <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-1.5 leading-relaxed">{{ $d['idee'] }}</p>
                </div>
            @endforeach
        </div>

        <p class="text-[11px] text-gray-400">Vollständiger Plan: <code class="font-mono">docs/14_ROADMAP_PHASE2.md</code> im Modul-Repo.</p>
    </x-ui-page-container>
</x-ui-page>
