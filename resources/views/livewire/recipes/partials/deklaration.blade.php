...
<div data-deklaration>
    ...
    <div class="grid grid-cols-2 gap-x-4 gap-y-0.5 text-xs" data-deklaration-diaet>
        @foreach([
            ...
            <span class="font-medium uppercase tracking-wide text-[11px] {{ $wert === true ? 'text-green-600' : ($wert === false ? 'text-gray-500 line-through decoration-1' : 'text-gray-500 italic') }}"
...
<div data-deklaration-allergene>
    <p class="{{ $dt }} mb-1">Allergene
        <span class="normal-case ml-1 font-semibold {{ ['high' => 'text-green-600', 'medium' => 'text-amber-500', 'low' => 'text-rose-500'][$rezept->allergens_confidence] ?? 'text-gray-500' }}">{{ strtoupper($rezept->allergens_confidence) }}</span>
    </p>
    <div class="grid grid-cols-2 gap-x-4 gap-y-0.5" data-allergen-grid>
        @foreach(\Platform\FoodAlchemist\Models\FoodAlchemistItemAllergen::ALLERGENE as $feld => $lbl)
            ...
            <span class="text-xs px-1.5 py-0.5 rounded {{ [
                    'enthalten' => 'bg-rose-500/10 text-rose-600 font-medium',
                    'spuren' => 'bg-amber-500/10 text-amber-600',
                    'nicht_enthalten' => 'text-gray-500',
                ][$wert] ?? 'text-gray-500 italic opacity-60' }}"
    ...
    <div class="grid grid-cols-2 gap-1" data-zusatz-grid>
        @foreach(\Platform\FoodAlchemist\Models\FoodAlchemistItemDeclaration::STOFFE as $stoff => $lbl)
            ...
            <span class="text-[11px] px-1.5 py-0.5 rounded border truncate {{ $wert === null
                    ? 'border-dashed border-black/15 text-gray-500 italic'
                    : ((int) $wert === 3
                        ? 'border-rose-300 bg-rose-500/10 text-rose-600 font-medium'
