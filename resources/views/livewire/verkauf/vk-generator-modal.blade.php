...
<x-foodalchemist::modal name="vk-generator-modal" title="Gericht generieren" size="max-w-2xl">
    ...
    @if($laeuft && $ergebnis === null)
        ...
        <div wire:poll.2s="pruefeErgebnis" class="py-6" data-vk-generator-laeuft>
            <div class="flex items-center gap-3 justify-center text-sm text-gray-600">
                ...
                <span data-vk-generator-fortschritt>{{ $fortschritt ?? 'Gericht wird generiert — das Fenster kann offen bleiben.' }}</span>
            </div>
            {{-- Phase 0 Watchdog: pending zu lange → sichtbarer Hinweis statt endlosem Spinner --}}
            @if($hinweis !== null)
                <p class="mt-3 text-[11px] text-amber-600 text-center" data-vk-generator-hinweis>⏱ {{ $hinweis }}</p>
            @endif
        </div>
    @elseif($ergebnis === null)
        ...
        <x-foodalchemist::modal-section title="Richtungs-Parameter">
            ...
            <div class="grid md:grid-cols-2 gap-x-6 gap-y-4 mt-4" data-vk-generator-parameter>
                ...
                <div class="md:col-span-2" data-richtung="favoriten">
                    <label class="flex items-start gap-2 text-xs font-medium text-gray-900">
                        <input type="checkbox" wire:model.live="useFavoritesList" class="mt-0.5" data-vk-favoriten />
    ...
    <x-slot:footer>
        ...
        @elseif(!$freigegeben)
            <button type="button" wire:click="generatorFreigeben" @disabled(count($ergebnis['offene'] ?? []) > 0)
                    class="{{ $btnPrimary }} {{ count($ergebnis['offene'] ?? []) > 0 ? 'opacity-50 cursor-not-allowed' : '' }}" data-vk-generator-freigeben>Gericht freigeben</button>
        @endif
    </x-slot:footer>
