    ...
    @else
        <div>
            ...
            @if($rezept->sales_wording_standard !== null)
                <p class="text-[11px] italic text-gray-500 dark:text-gray-400 mt-0.5">{{ $rezept->sales_wording_standard }}</p>
            @endif
        ...
        @if($klasseVorschlag !== null)
            <div class="rounded-lg bg-violet-500/10 border border-violet-500/30 px-3 py-2 text-xs" data-klasse-vorschlag>
                <p class="text-gray-900 dark:text-gray-100">
                    ...
                    <span class="text-[11px] text-gray-500 dark:text-gray-400">· {{ round($klasseVorschlag['confidence'] * 100) }} %</span>
                </p>
                @if($klasseVorschlag['reasoning'] !== null)<p class="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">{{ $klasseVorschlag['reasoning'] }}</p>@endif
        ...
        @if($rollenVorschlag !== null)
            <div class="rounded-lg bg-violet-500/10 border border-violet-500/30 px-3 py-2 text-xs" data-rollen-vorschlag>
                ...
                @if($rollenVorschlag['rollen'] === [])
                    <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">Kein gültiger Vorschlag (Rollen-Vokabular: aroma_treiber · komponente · beilage · garnitur).</p>
                @else
        ...
        @elseif($rezept->markupClass !== null && $cockpit['vk']['vorschlag'] !== null)
            <p class="text-[11px] text-gray-500 dark:text-gray-400" data-formel-klartext>{{ $rezept->markupClass->code }} · {{ $rezept->markupClass->label }} · {{ $cockpit['vk']['vorschlag']['formel'] }}</p>
        @elseif($rezept->ek_total_eur === null)
            <p class="text-[11px] text-gray-500 dark:text-gray-400" data-cockpit-leer>Kein EK berechnet — Zutaten ergänzen oder Lead-LAs setzen.</p>
        @elseif($rezept->markupClass === null)
            <p class="text-[11px] text-gray-500 dark:text-gray-400" data-cockpit-leer>Keine Aufschlagsklasse gesetzt — VK-Vorschlag erst nach Klassifikation (M6-04).</p>
        @endif
        ...
        <div data-vk-eignung>
            ...
            @foreach(['sektor' => ['Sektor', $sektorEignungen, 'sector_slug'], 'level' => ['Niveau', $niveauEignungen, 'level_slug']] as $typ => [$lbl, $eignungen, $slugSpalte])
                <div class="flex items-center gap-1.5 flex-wrap py-0.5" data-eignung-zeile="{{ $typ }}">
                    <span class="text-[11px] text-gray-500 dark:text-gray-400 w-12 shrink-0">{{ $lbl }}</span>
                    ...
                    @empty
                        <span class="text-[11px] text-gray-500 dark:text-gray-400 italic">— keine —</span>
                    @endforelse
            ...
            @if(($offen['kohaerenz'] ?? false) && $kohaerenzStatus !== null)
                ...
                @if($urteil?->score !== null)
                    <div class="mt-1 space-y-1.5" data-vk-kohaerenz-urteil>
                        ...
                        @endif
                        <p class="text-[11px] text-gray-500 dark:text-gray-400">{{ $urteil->judged_at?->format('Y-m-d') }} · {{ $urteil->judge_model }}
                            @if($kohaerenzStatus['stale'])<span class="text-amber-500" data-vk-kohaerenz-stale> · Zutaten geändert — Urteil veraltet</span>@endif
                        </p>
                    </div>
                @else
                    <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-1">Noch kein Urteil — der Judge beurteilt die Stimmigkeit des Tellers (zweite Achse neben der Aroma-Kohäsion).</p>
                @endif
                            ...
                            @foreach(['kontrast' => 'Kontrast', 'ergaenzung' => 'Ergänzung', 'veredelung' => 'Veredelung'] as $t => $lbl)
                                @if(($typen[$t] ?? 0) > 0)
                                    <button type="button" @click="typ = '{{ $t }}'"
                                            ...
                                            :class="typ === '{{ $t }}' ? 'border-amber-500 text-amber-700 dark:text-amber-300 font-semibold' : 'border-black/10 dark:border-white/15 text-gray-500 dark:text-gray-400'">{{ $lbl }} {{ $typen[$t] }}</button>
                                @endif
                            @endforeach
                        </div>
                            ...
                            <div x-show="typ === '{{ $v['type'] }}'" class="flex gap-2 text-xs" wire:key="vkh-{{ $loop->index }}">
                                ...
                                <div class="min-w-0">
                                    <p class="font-medium text-gray-900 dark:text-gray-100">{{ $v['zutat'] }}
                                        @if($v['category'] !== null)<span class="text-[11px] font-normal text-gray-500 dark:text-gray-400 ml-1">{{ $v['category'] }}</span>@endif
                                    </p>
                                    @if($v['reasoning'] !== null)<p class="text-[11px] text-gray-600 dark:text-gray-400 leading-relaxed">{{ $v['reasoning'] }}</p>@endif
                                </div>
                            </div>
                        @endforeach
                        <p class="text-[11px] text-gray-500 dark:text-gray-400">{{ $kohaerenzStatus['cache']->heber_at?->format('Y-m-d') }} · {{ $kohaerenzStatus['cache']->heber_model }}</p>
                    </div>
                @else
                    <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-1">Noch keine Vorschläge — die KI nennt 1–3 machbare Hebel (Kontrast / Ergänzung / Veredelung).</p>
                @endif
        ...
        <div data-vk-nachbarn>
            ...
            @if(($offen['nachbarn'] ?? false) && $nachbarn !== null)
                <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-1">Aromaverwandte Zutaten aus dem Geschmacks-Netz — Inspiration, kein Teller-Urteil.</p>
                @if($nachbarn['klassiker'] === [])
                    <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-1" data-vk-nachbarn-leer>Zu wenige aufgelöste Anker (mind. 2 nötig) — Kern-Anker setzen hilft.</p>
                @else
                    <div class="mt-1.5 space-y-2" x-data="{ modus: 'klassiker' }" data-vk-nachbarn-liste>
                        <div class="flex items-center gap-1.5">
                            ...
                            <span class="text-[10px] text-gray-500 dark:text-gray-400" x-text="modus === 'klassiker' ? 'trifft viele Komponenten (sicher)' : 'spezifisch statt promiskuitiv (eigen)'"></span>
                        </div>
                                    ...
                                    <div class="flex gap-2 text-xs" wire:key="vkn-{{ $modus }}-{{ $n['anchor_id'] }}">
                                        ...
                                        <div class="min-w-0">
                                            <p class="font-medium text-gray-900 dark:text-gray-100">{{ $n['slug'] }}
                                                @if($n['allrounder'])<span class="text-[11px] font-normal text-gray-500 dark:text-gray-400 ml-1">Allrounder</span>@endif
                                            </p>
                                            <p class="text-[11px] text-gray-600 dark:text-gray-400 truncate">verbindet {{ $n['cover'] }}/{{ $n['dish_n'] }}: {{ implode(', ', $n['trifft']) }}</p>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
            ...
            <div class="space-y-1" data-vk-zutaten>
                @foreach($rezept->ingredients as $z)
                    <p class="text-xs text-gray-900 dark:text-gray-100 leading-snug" wire:key="vkz-{{ $z->id }}">
                        <span class="text-gray-500 dark:text-gray-400 tabular-nums">{{ $z->quantity !== null ? rtrim(rtrim(number_format((float) $z->quantity, 2, ',', '.'), '0'), ',') . ' ' . ($z->unit?->slug ?? '') : '' }}</span>
