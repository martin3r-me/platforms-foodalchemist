    ...
    @else
        <div>
            ...
            @if($rezept->sales_wording_standard !== null)
                <p class="text-[11px] italic text-gray-500 mt-0.5">{{ $rezept->sales_wording_standard }}</p>
            @endif
            <div class="flex flex-wrap items-center gap-1.5 mt-2" data-vk-aktionen>
                ...
                <button type="button" wire:click="ai_rollen" class="{{ $btnGhostXs }} text-violet-600" title="ai_verteile_rollen — Gesamt-Gericht-Sicht (V-21)" data-vk-rollen>🎭 Rollen</button>
            </div>
        ...
        @if($klasseVorschlag !== null)
            <div class="rounded-lg bg-violet-500/10 border border-violet-500/30 px-3 py-2 text-xs" data-klasse-vorschlag>
                <p class="text-gray-900">
                    ...
                    <span class="text-[11px] text-gray-500">· {{ round($klasseVorschlag['confidence'] * 100) }} %</span>
                </p>
        ...
        @if($rollenVorschlag !== null)
            <div class="rounded-lg bg-violet-500/10 border border-violet-500/30 px-3 py-2 text-xs" data-rollen-vorschlag>
                ...
                @if($rollenVorschlag['rollen'] === [])
                    <p class="text-[11px] text-gray-500 mt-0.5">Kein gültiger Vorschlag (Rollen-Vokabular: aroma_treiber · komponente · beilage · garnitur).</p>
                @else
                    <div class="mt-1 space-y-0.5">
                        @foreach($rollenVorschlag['rollen'] as $zeileId => $role)
                            @php($zeile = $rezept->ingredients->firstWhere('id', $zeileId))
                            <p class="text-[11px] text-gray-600" wire:key="rv-{{ $zeileId }}">
        ...
        <div class="grid grid-cols-2 gap-2" data-vk-kpis>
            ...
            <div class="rounded-lg bg-black/[0.03] px-3 py-2">
                ...
                <p class="text-xs font-semibold text-gray-900" data-vk-netto>{{ $cockpit['vk']['sales_net'] !== null ? number_format($cockpit['vk']['sales_net'], 2, ',', '.') . ' €' : '—' }}</p>
            </div>
            <div class="rounded-lg bg-violet-500/10 border border-violet-500/30 px-3 py-2" data-vk-brutto>
                ...
                <p class="text-base font-bold text-violet-700">{{ $cockpit['sales_gross'] !== null ? number_format($cockpit['sales_gross'], 2, ',', '.') . ' €' : '—' }}</p>
            </div>
            <div class="rounded-lg bg-black/[0.03] px-3 py-2">
                <span class="{{ $dt }}">Wareneinsatz</span>
                <p class="text-xs font-semibold {{ ($cockpit['marge']['wareneinsatz_pct'] ?? 0) > 35 ? 'text-rose-600' : 'text-gray-900' }}" data-wareneinsatz>
            ...
            @if($cockpit['pro_einheit'] !== null)
                <div class="rounded-lg bg-black/[0.03] px-3 py-2">
                    ...
                    <p class="text-xs font-semibold text-gray-900">{{ number_format($cockpit['pro_einheit']['vk_netto_pro_einheit'], 2, ',', '.') }} €</p>
                </div>
                <div class="rounded-lg bg-black/[0.03] px-3 py-2">
                    <span class="{{ $dt }}">VK brutto/Einheit</span>
                    <p class="text-xs font-semibold text-gray-900">{{ number_format($cockpit['pro_einheit']['vk_brutto_pro_einheit'], 2, ',', '.') }} €</p>
                </div>
            @endif
        </div>
        ...
        @if($cockpit['formel_fehlt'])
            <p class="text-[11px] text-amber-600" data-formel-fehlt>⚠ Aufschlagsklasse {{ $rezept->markupClass?->code }}: Formel »deckungsbeitrag« nicht definiert (W-1) — Entscheid ausstehend, VK nur manuell.</p>
        @elseif($rezept->markupClass !== null && $cockpit['vk']['vorschlag'] !== null)
            <p class="text-[11px] text-gray-500" data-formel-klartext>{{ $rezept->markupClass->code }} · {{ $rezept->markupClass->label }} · {{ $cockpit['vk']['vorschlag']['formel'] }}</p>
        @elseif($rezept->ek_total_eur === null)
            <p class="text-[11px] text-gray-500" data-cockpit-leer>Kein EK berechnet — Zutaten ergänzen oder Lead-LAs setzen.</p>
        @elseif($rezept->markupClass === null)
            <p class="text-[11px] text-gray-500" data-cockpit-leer>Keine Aufschlagsklasse gesetzt — VK-Vorschlag erst nach Klassifikation (M6-04).</p>
        @endif

        @if($rezept->description !== null)
            <div>
                <p class="{{ $dt }} mb-1">Beschreibung</p>
                <p class="text-xs text-gray-600 leading-relaxed">{{ $rezept->description }}</p>
            </div>
        @endif
        ...
        <div data-vk-eignung>
            <p class="{{ $dt }} mb-1 flex items-center gap-2">Eignung
                <button type="button" wire:click="kiEignung" class="{{ $btnGhostXs }} text-violet-600 ml-auto normal-case" title="recipe.sektor + recipe.level — nur «geeignet»-Urteile werden Vorschlag" data-ki-eignung>✨ Eignung</button>
            </p>
            ...
            @foreach(['sektor' => ['Sektor', $sektorEignungen, 'sector_slug'], 'level' => ['Niveau', $niveauEignungen, 'level_slug']] as $typ => [$lbl, $eignungen, $slugSpalte])
                <div class="flex items-center gap-1.5 flex-wrap py-0.5" data-eignung-zeile="{{ $typ }}">
                    <span class="text-[11px] text-gray-500 w-12 shrink-0">{{ $lbl }}</span>
                    ...
                    @empty
                        <span class="text-[11px] text-gray-500 italic">— keine —</span>
                    @endforelse
        ...
        <div data-vk-kern-anker>
            ...
            @if($offen['anker'] ?? false)
                ...
                @if($kohaesion !== null)
                    <div class="mt-2 rounded-lg bg-black/[0.03] px-3 py-2 text-[11px] space-y-0.5" data-vk-kohaesion>
                ...
                @if($urteil?->score !== null)
                    <div class="mt-1 space-y-1.5" data-vk-kohaerenz-urteil>
                        <div class="flex items-center gap-3">
                            <span class="text-2xl font-semibold {{ $urteil->score >= 80 ? 'text-green-600' : ($urteil->score >= 50 ? 'text-amber-600' : 'text-rose-600') }}">{{ $urteil->score }} %</span>
                            @if($urteil->label !== null)
                                <span class="px-2.5 py-0.5 rounded-full border text-[10px] font-semibold uppercase tracking-wider {{ $urteil->score >= 80 ? 'border-green-600 text-green-700' : 'border-amber-500 text-amber-700' }}">{{ $urteil->label }}</span>
                            @endif
                        </div>
                        @if($urteil->reasoning !== null)
                            <p class="text-xs text-gray-600 leading-relaxed">{{ $urteil->reasoning }}</p>
                        @endif
                        @if($urteil->schwachstelle !== null)
                            <p class="text-[11px] text-amber-600">Schwachstelle: {{ $urteil->schwachstelle }}</p>
                        @endif
                        <p class="text-[11px] text-gray-500">{{ $urteil->judged_at?->format('Y-m-d') }} · {{ $urteil->judge_model }}
                            @if($kohaerenzStatus['stale'])<span class="text-amber-500" data-vk-kohaerenz-stale> · Zutaten geändert — Urteil veraltet</span>@endif
                        </p>
                    </div>
                @else
                    <p class="text-[11px] text-gray-500 mt-1">Noch kein Urteil — der Judge beurteilt die Stimmigkeit des Tellers (zweite Achse neben der Aroma-Kohäsion).</p>
                @endif
                ...
                @if($heber !== null && ($heber['vorschlaege'] ?? []) !== [])
                    ...
                    <div class="mt-1 space-y-2" x-data="{ typ: '{{ collect($heber['vorschlaege'])->first()['type'] }}' }" data-vk-heber-vorschlaege>
                        @if(($heber['einschaetzung'] ?? null) !== null)
                            <p class="text-xs font-medium text-gray-900 leading-relaxed">{{ $heber['einschaetzung'] }}</p>
                        @endif
                            ...
                            @foreach(['kontrast' => 'Kontrast', 'ergaenzung' => 'Ergänzung', 'veredelung' => 'Veredelung'] as $t => $lbl)
                                @if(($typen[$t] ?? 0) > 0)
                                    <button type="button" @click="typ = '{{ $t }}'"
                                            ...
                                            :class="typ === '{{ $t }}' ? 'border-amber-500 text-amber-700 font-semibold' : 'border-black/10 text-gray-500'">{{ $lbl }} {{ $typen[$t] }}</button>
                                @endif
                            @endforeach
                        </div>
                        @foreach($heber['vorschlaege'] as $v)
                            <div x-show="typ === '{{ $v['type'] }}'" class="flex gap-2 text-xs" wire:key="vkh-{{ $loop->index }}">
                                @if($v['confidence'] !== null)<span class="font-semibold text-green-600 shrink-0">{{ round($v['confidence'] * 100) }} %</span>@endif
                                <div class="min-w-0">
                                    <p class="font-medium text-gray-900">{{ $v['zutat'] }}
                                        @if($v['category'] !== null)<span class="text-[11px] font-normal text-gray-500 ml-1">{{ $v['category'] }}</span>@endif
                                    </p>
                                    @if($v['reasoning'] !== null)<p class="text-[11px] text-gray-600 leading-relaxed">{{ $v['reasoning'] }}</p>@endif
                                </div>
                            </div>
                        @endforeach
                        <p class="text-[11px] text-gray-500">{{ $kohaerenzStatus['cache']->heber_at?->format('Y-m-d') }} · {{ $kohaerenzStatus['cache']->heber_model }}</p>
                    </div>
                @else
                    <p class="text-[11px] text-gray-500 mt-1">Noch keine Vorschläge — die KI nennt 1–3 machbare Hebel (Kontrast / Ergänzung / Veredelung).</p>
                @endif
        ...
        <div data-vk-nachbarn>
            ...
            @if(($offen['nachbarn'] ?? false) && $nachbarn !== null)
                <p class="text-[11px] text-gray-500 mt-1">Aromaverwandte Zutaten aus dem Geschmacks-Netz — Inspiration, kein Teller-Urteil.</p>
                @if($nachbarn['klassiker'] === [])
                    <p class="text-[11px] text-gray-500 mt-1" data-vk-nachbarn-leer>Zu wenige aufgelöste Anker (mind. 2 nötig) — Kern-Anker setzen hilft.</p>
                @else
                    <div class="mt-1.5 space-y-2" x-data="{ modus: 'klassiker' }" data-vk-nachbarn-liste>
                        <div class="flex items-center gap-1.5">
                            ...
                            <span class="text-[10px] text-gray-500" x-text="modus === 'klassiker' ? 'trifft viele Komponenten (sicher)' : 'spezifisch statt promiskuitiv (eigen)'"></span>
                        </div>
                            ...
                            <div x-show="modus === '{{ $modus }}'" class="space-y-1.5">
                                @foreach($nachbarn[$modus] as $n)
                                    <div class="flex gap-2 text-xs" wire:key="vkn-{{ $modus }}-{{ $n['anchor_id'] }}">
                                        <span class="font-semibold shrink-0 {{ $n['mean_w'] >= 80 ? 'text-green-600' : 'text-blue-600' }}">{{ $n['mean_w'] }} %</span>
                                        <div class="min-w-0">
                                            <p class="font-medium text-gray-900">{{ $n['slug'] }}
                                                @if($n['allrounder'])<span class="text-[11px] font-normal text-gray-500 ml-1">Allrounder</span>@endif
                                            </p>
                                            <p class="text-[11px] text-gray-600 truncate">verbindet {{ $n['cover'] }}/{{ $n['dish_n'] }}: {{ implode(', ', $n['trifft']) }}</p>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
        ...
        <div>
            ...
            <div class="space-y-1" data-vk-zutaten>
                @foreach($rezept->ingredients as $z)
                    <p class="text-xs text-gray-900 leading-snug" wire:key="vkz-{{ $z->id }}">
                        <span class="text-gray-500 tabular-nums">{{ $z->quantity !== null ? rtrim(rtrim(number_format((float) $z->quantity, 2, ',', '.'), '0'), ',') . ' ' . ($z->unit?->slug ?? '') : '' }}</span>
