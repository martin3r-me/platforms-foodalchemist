...
<div class="space-y-4" data-settings-herstellkosten>
    ...
    <div class="{{ $card }} p-5 space-y-3" data-hk-schema>
        <div class="flex items-start justify-between gap-3 flex-wrap">
            <div>
                ...
                <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5"><strong>MEK + MGK + FEK + FGK = HK → +Verwaltung/Logistik = Selbstkosten (HK2) → × Marge = VK-Vorschlag.</strong> Gemeinkosten stehen auf <em>automatisch</em> — du trägst unten deine Fixkosten in € ein, der Zuschlag-% rechnet sich selbst (Σ Fixkosten ÷ Bezugsbasis). <em>manuell (%)</em> nur als Ausnahme.</p>
            </div>
                ...
                @foreach($schema as $i => $b)
                    ...
                    <tr wire:key="kblock-{{ $b['key'] }}" class="{{ $tr }}">
                        ...
                        <td class="{{ $td }}">
                            ...
                            @else
                                <span class="text-[11px] text-gray-500 dark:text-gray-400">direkt</span>
                            @endif
                        </td>
                        <td class="{{ $td }} text-right">
                            @if($istAbgeleitet)
                                ...
                                @if($blockSumme > 0 && $blockBasis > 0)
                                    <span class="block text-[10px] text-gray-500 dark:text-gray-400 tabular-nums" title="Σ Fixkosten/Monat ÷ Bezugsbasis">{{ number_format($blockSumme, 0, ',', '.') }} € ÷ {{ number_format($blockBasis, 0, ',', '.') }} €</span>
                                @elseif($basisFehlt)
                                ...
                                @else
                                    <span class="block text-[10px] text-gray-500 dark:text-gray-400">noch keine Fixkosten</span>
                                @endif
                            @else
                                <input type="text" wire:model="schema.{{ $i }}.wert" class="{{ $input }} !w-24 text-right tabular-nums" placeholder="0" />
                                <span class="text-[10px] text-gray-500 dark:text-gray-400">{{ $b['type'] === 'eur_pro_portion' ? '€' : ($b['type'] === 'arbeitszeit' ? '€/h' : '%') }}</span>
                            @endif
                        </td>
                        <td class="{{ $td }} text-right">
                            <button type="button" wire:click="blockEntfernen({{ $i }})" wire:confirm="Kostenblock entfernen?" class="text-gray-500 dark:text-gray-400 hover:text-red-500" title="Block entfernen">✕</button>
                        </td>
                    </tr>
                @endforeach
                {{-- Neuer Block --}}
        ...
        <div class="flex items-center gap-3 pt-1 border-t border-black/5 dark:border-white/10">
            ...
            <input type="text" wire:model="marge" class="{{ $input }} !w-24 text-right tabular-nums" placeholder="15" /> <span class="text-[11px] text-gray-500 dark:text-gray-400">% auf HK2</span>
        </div>
        <div class="flex items-center gap-3">
            <span class="w-40 text-xs text-gray-600 dark:text-gray-300">Ziel-Wareneinsatzquote</span>
            <input type="text" wire:model="zielWe" class="{{ $input }} !w-24 text-right tabular-nums" placeholder="30" /> <span class="text-[11px] text-gray-500 dark:text-gray-400">% Food-Cost-Ziel (gastro-üblich 28–35 %) — treibt Break-even + Signal „Wareneinsatz über Ziel"</span>
        </div>
        <div class="flex items-center gap-3">
            <span class="w-40 text-xs text-gray-600 dark:text-gray-300">Lohnnebenkosten-Zuschlag</span>
            <input type="text" wire:model="lnk" class="{{ $input }} !w-24 text-right tabular-nums" placeholder="0" /> <span class="text-[11px] text-gray-500 dark:text-gray-400">% AG-/Sozialabgaben auf den Produktionslohn — rechnet den <strong>echten</strong> Personalkostensatz (statt nur Brutto-Lohn) in HK2</span>
        </div>
    </div>
    ...
    <div class="{{ $card }} p-5 space-y-3" data-hk-fixkosten>
        <div>
            ...
            <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">Nicht-produktbezogene Kosten (Logistik, Spüle, Lager, Verwaltung …). <strong>Zuschlag-% = Σ Fixkosten/Monat ÷ Bezugsbasis × 100</strong> für jeden Block im Modus „aus Fixkosten".</p>
        </div>
        ...
        <div class="rounded-lg bg-black/[0.03] dark:bg-white/5 p-3 space-y-2">
            <p class="text-[11px] text-gray-600 dark:text-gray-400">Bezugsbasen = die erwarteten <strong>Monatswerte</strong>, durch die die Fixkosten geteilt werden. Faustregel: Ø der letzten 3 Monate (Ist) oder Planwert. <em>0 = Block bleibt 0 %.</em></p>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                @foreach([
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-1"><input type="text" wire:model.live.debounce.600ms="bezugsbasen.{{ $k }}" class="{{ $input }} text-right tabular-nums" placeholder="0" /> <span class="text-[11px] text-gray-500 dark:text-gray-400">€</span></div>
                        <p class="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">{{ $hint }}</p>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <tbody>
                @forelse($fixListe as $f)
                    <tr wire:key="fix-{{ $f['id'] }}" class="{{ $tr }}">
                        ...
                        <td class="{{ $td }} text-gray-600 dark:text-gray-400">{{ collect($gkBloecke)->firstWhere('key', $f['block_key'])['label'] ?? $f['block_key'] }}</td>
                        <td class="{{ $td }} text-right tabular-nums">{{ number_format((float) $f['amount'], 2, ',', '.') }} €</td>
                        <td class="{{ $td }} text-gray-600 dark:text-gray-400">
                            {{ $f['periode'] === 'jaehrlich' ? 'jährlich' : 'monatlich' }}
                            @if($f['periode'] === 'jaehrlich')
                                <span class="text-[10px] text-gray-500 dark:text-gray-400">= {{ number_format((float) ($f['monatsbetrag'] ?? 0), 2, ',', '.') }} €/Mt</span>
                            @endif
                        </td>
                        <td class="{{ $td }} text-right"><button type="button" wire:click="fixLoeschen({{ $f['id'] }})" wire:confirm="Fixkosten-Zeile löschen?" class="text-gray-500 dark:text-gray-400 hover:text-red-500">✕</button></td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="px-3 py-4 text-center text-[11px] text-gray-500 dark:text-gray-400">Noch keine Fixkosten erfasst.</td></tr>
                @endforelse
                {{-- Neue Zeile --}}
                <tr class="border-t-2 border-black/5 dark:border-white/10">
        ...
        <div class="flex flex-col gap-1 pt-2 border-t border-black/5 dark:border-white/10">
            ...
            @if($fixMonatGesamt > 0)
                ...
                <p class="text-[10px] text-gray-500 dark:text-gray-400">Davon fließen pro Block ÷ Bezugsbasis die abgeleiteten Zuschlag-% (Modus „aus Fixkosten").</p>
            @endif
        </div>
    </div>
