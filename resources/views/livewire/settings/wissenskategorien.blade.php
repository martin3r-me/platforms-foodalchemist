...
<div class="space-y-5" data-settings-wissenskategorien>
    <div>
        ...
        <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">Klassifiziert Wissens-Dokumente (such-/filterbar) und trägt die grobe Routing-Ebene (Feature × Kategorie). Löschen nur, wenn keine Dokumente die Kategorie nutzen — sonst deaktivieren.</p>
    </div>
            ...
            @foreach($kategorien as $kat)
                <tr class="{{ $tr }} {{ $kat->active ? '' : 'opacity-50' }}" wire:key="kat-{{ $kat->id }}">
                    @if($editId === $kat->id)
                        ...
                        <td class="{{ $td }} text-[10px] font-mono text-gray-500 dark:text-gray-400">{{ $kat->slug }}</td>
                        <td class="{{ $td }}"><input type="text" wire:model="form.description" class="{{ $input }} !py-1 w-full" placeholder="Beschreibung (optional)" /></td>
                        <td class="{{ $td }} text-[11px] text-gray-500 dark:text-gray-400">{{ $docCounts[$kat->slug] ?? 0 }}</td>
                    ...
                    @else
                        ...
                        <td class="{{ $td }} text-[10px] font-mono text-gray-500 dark:text-gray-400">{{ $kat->slug }}</td>
                        <td class="{{ $td }} text-[11px] text-gray-500 dark:text-gray-400 max-w-[16rem] truncate">{{ $kat->description ?? '—' }}</td>
                        <td class="{{ $td }} text-[11px] text-gray-600 dark:text-gray-400">{{ $docCounts[$kat->slug] ?? 0 }}</td>
                        <td class="{{ $td }} text-[11px] text-gray-500 dark:text-gray-400">{{ $kat->sort_order }}</td>
                        <td class="{{ $td }} whitespace-nowrap">
    ...
    <div class="rounded-lg bg-black/[0.03] dark:bg-white/5 px-3 py-2 space-y-1.5" data-kat-anlegen>
        ...
        <p class="text-[10px] text-gray-500 dark:text-gray-400">Slug wird aus dem Label erzeugt.</p>
    </div>
</div>
