...
<div class="space-y-5" data-settings-wissenskategorien>
    <div>
        <h3 class="font-medium tracking-tight text-gray-900">Wissens-Kategorien</h3>
        <p class="text-[11px] text-gray-500 mt-0.5">Klassifiziert Wissens-Dokumente (such-/filterbar) und trägt die grobe Routing-Ebene (Feature × Kategorie). Löschen nur, wenn keine Dokumente die Kategorie nutzen — sonst deaktivieren.</p>
    </div>
    @if($fehler !== null)<p class="text-xs text-rose-600" data-kat-fehler>{{ $fehler }}</p>@endif
            ...
            @foreach($kategorien as $kat)
                <tr class="{{ $tr }} {{ $kat->active ? '' : 'opacity-50' }}" wire:key="kat-{{ $kat->id }}">
                    @if($editId === $kat->id)
                        ...
                        <td class="{{ $td }} text-[10px] font-mono text-gray-500">{{ $kat->slug }}</td>
                    ...
                    @else
                        ...
                        <td class="{{ $td }} text-[10px] font-mono text-gray-500">{{ $kat->slug }}</td>
                        <td class="{{ $td }} text-[11px] text-gray-500 max-w-[16rem] truncate">{{ $kat->description ?? '—' }}</td>
                        <td class="{{ $td }} text-[11px] text-gray-600">{{ $docCounts[$kat->slug] ?? 0 }}</td>
                        <td class="{{ $td }} text-[11px] text-gray-500">{{ $kat->sort_order }}</td>
                        <td class="{{ $td }} whitespace-nowrap">
    ...
    {{-- Anlegen --}}
    <div class="rounded-lg bg-black/[0.03] px-3 py-2 space-y-1.5" data-kat-anlegen>
        ...
        <p class="text-[10px] text-gray-500">Slug wird aus dem Label erzeugt.</p>
    </div>
</div>
