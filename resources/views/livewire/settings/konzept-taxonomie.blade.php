...
<div class="space-y-4">
    ...
    <div class="flex items-center gap-1.5">
        ...
        <span class="text-[10px] text-gray-500 ml-2 leading-snug">Rein organisatorisch — Filter-/Gruppier-Achse im Concept-Browser &amp; Foodbook/Angebots-Picker, ohne Preis-Einfluss.</span>
    </div>
                ...
                @forelse($topKats as $kat)
                    ...
                    <div wire:key="ktop-{{ $kat['id'] }}" class="group flex items-center gap-1 rounded-lg {{ $katSelectedId === $kat['id'] ? $katAktiv : $katHover }}">
                        @if($editKatId === $kat['id'])
                            ...
                            <button type="button" wire:click="katRename" class="{{ $btnGhostXs }} text-violet-600 shrink-0">OK</button>
                        @else
                            ...
                            <button type="button" wire:click="katLoeschen({{ $kat['id'] }})" wire:confirm="Kategorie „{{ $kat['name'] }}" löschen? (Unterkategorien & Concepts rücken zum Eltern)" class="shrink-0 opacity-0 group-hover:opacity-100 text-gray-500 hover:text-red-500 text-[11px] px-1" title="Löschen">@svg('heroicon-o-trash', 'w-3.5 h-3.5 inline-block align-middle')</button>
                        @endif
                    </div>
                @empty
                    <p class="text-[11px] text-gray-500 px-2 py-2">Noch keine Kategorien.</p>
                @endforelse
                <div class="flex gap-1 pt-2 mt-1 border-t border-black/5">
            ...
            <div class="flex-1 min-w-0">
                <div class="relative overflow-hidden {{ $card }}" data-konzept-kat-sub>
                    ...
                    <div class="px-5 pt-4 pb-2 flex items-baseline justify-between">
                        <h3 class="font-medium tracking-tight text-gray-900">Unterkategorien</h3>
                    ...
                    @if($katSelectedId !== null)
                        <table class="{{ $table }}">
                            ...
                            <tbody>
                                    ...
                                    <tr wire:key="ksub-{{ $sub['id'] }}" class="{{ $tr }}">
                                        @if($editKatId === $sub['id'])
                                            ...
                                            <td class="{{ $td }} text-right whitespace-nowrap">
                                                <button type="button" wire:click="katRename" class="{{ $btnGhostXs }} text-violet-600">Speichern</button>
                                                <button type="button" wire:click="$set('editKatId', null)" class="{{ $btnGhostXs }}">Abbrechen</button>
                                            </td>
                                        @else
                                            <td class="{{ $td }} text-gray-900">{{ $sub['name'] }}</td>
                                            <td class="{{ $td }} text-gray-600">{{ $katCounts[$sub['id']] ?? 0 }}</td>
                                            <td class="{{ $td }} text-right whitespace-nowrap">
                                ...
                                @empty
                                    <tr><td colspan="3" class="{{ $td }} text-gray-500 py-4 text-center">Noch keine Unterkategorien — unten anlegen.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                        <div class="px-5 py-3 border-t border-black/5 flex items-center gap-1.5">
                    ...
                    @else
                        <div class="px-5 pb-5 text-xs text-gray-500">Links eine Kategorie wählen, um ihre Unterkategorien zu sehen und zu pflegen.</div>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-4 items-start">
                ...
                @forelse($topKl as $kl)
                    ...
                    <div wire:key="kltop-{{ $kl['id'] }}" class="group flex items-center gap-1 rounded-lg {{ $klasseSelectedId === $kl['id'] ? $katAktiv : $katHover }}">
                        @if($editKlasseId === $kl['id'])
                            ...
                            <button type="button" wire:click="klasseRename" class="{{ $btnGhostXs }} text-violet-600 shrink-0">OK</button>
                        @else
                            ...
                            <button type="button" wire:click="klasseLoeschen({{ $kl['id'] }})" wire:confirm="Klasse „{{ $kl['name'] }}" löschen? (Unterklassen rücken zum Eltern)" class="shrink-0 opacity-0 group-hover:opacity-100 text-gray-500 hover:text-red-500 text-[11px] px-1" title="Löschen">@svg('heroicon-o-trash', 'w-3.5 h-3.5 inline-block align-middle')</button>
                        @endif
                    </div>
                @empty
                    <p class="text-[11px] text-gray-500 px-2 py-2">Noch keine Klassen.</p>
                @endforelse
                <div class="flex gap-1 pt-2 mt-1 border-t border-black/5">
            ...
            <div class="flex-1 min-w-0">
                <div class="relative overflow-hidden {{ $card }}" data-konzept-kl-sub>
                    <div class="{{ $cardAccent }}"></div>
                    <div class="px-5 pt-4 pb-2 flex items-baseline justify-between">
                        <h3 class="font-medium tracking-tight text-gray-900">Unterklassen</h3>
                    ...
                    @if($klasseSelectedId !== null)
                        <table class="{{ $table }}">
                            ...
                            <tbody>
                                @forelse($subKl as $sub)
                                    <tr wire:key="klsub-{{ $sub['id'] }}" class="{{ $tr }}">
                                        @if($editKlasseId === $sub['id'])
                                            ...
                                            <td class="{{ $td }}"></td>
                                            <td class="{{ $td }} text-right whitespace-nowrap">
                                                <button type="button" wire:click="klasseRename" class="{{ $btnGhostXs }} text-violet-600">Speichern</button>
                                                <button type="button" wire:click="$set('editKlasseId', null)" class="{{ $btnGhostXs }}">Abbrechen</button>
                                            </td>
                                        @else
                                            <td class="{{ $td }} text-gray-900">{{ $sub['name'] }}</td>
                                            <td class="{{ $td }} text-gray-600">{{ $klasseCounts[$sub['name']] ?? 0 }}</td>
                                            <td class="{{ $td }} text-right whitespace-nowrap">
                                ...
                                @empty
                                    <tr><td colspan="3" class="{{ $td }} text-gray-500 py-4 text-center">Noch keine Unterklassen — unten anlegen.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                        <div class="px-5 py-3 border-t border-black/5 flex items-center gap-1.5">
                    ...
                    @else
                        <div class="px-5 pb-5 text-xs text-gray-500">Links eine Klasse wählen, um ihre Unterklassen zu sehen und zu pflegen.</div>
                    @endif
                </div>
            </div>
        </div>
