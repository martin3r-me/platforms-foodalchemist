...
<div class="space-y-5" data-settings-ki>
    ...
    @if($meldung !== null)
        <p class="text-xs {{ $kiAktiv ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400' }}" data-ki-meldung>{{ $meldung }}</p>
    @endif
    @if(! $kiAktiv)
        <div class="rounded-lg bg-rose-500/10 border border-rose-500/30 px-3 py-2 text-xs text-rose-700 dark:text-rose-300" data-ki-aus-banner>
