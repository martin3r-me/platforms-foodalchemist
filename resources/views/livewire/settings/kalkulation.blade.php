...
<div class="space-y-4">
    ...
    <div class="{{ $card }} p-5 space-y-3" data-kalk-garverlust>
        <div>
            ...
            <p class="text-[11px] text-gray-400 mt-0.5">In % je GP-Klasse (Warengruppe). Kaskade: Zutat-Wert → GP-Default → dieser Team-Default → 0. Leer = kein Default.</p>
        </div>
    ...
    <div class="{{ $card }} p-5 space-y-3" data-kalk-putzverlust>
        ...
        <div class="flex items-center gap-3 py-1.5 border-b border-black/5 dark:border-white/5">
            <span class="w-72 shrink-0 text-xs font-medium text-gray-900 dark:text-gray-100">* Global (alle Klassen)</span>
            <input type="text" wire:model="putzverlust.*" placeholder="—" class="{{ $input }} !w-24" /> <span class="text-[11px] text-gray-400">%</span>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8">
            @foreach($warengruppen as $wg)
                <div class="flex items-center gap-3 py-1" wire:key="pv-{{ $wg->code }}">
                    <span class="w-64 shrink-0 text-xs text-gray-600 dark:text-gray-300 truncate">{{ $wg->name }}</span>
                    <input type="text" wire:model="putzverlust.{{ $wg->code }}" placeholder="—" class="{{ $input }} !w-20 !py-1" /> <span class="text-[11px] text-gray-400">%</span>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <p class="text-[11px] text-gray-400">Herstellkosten (Zuschlagsschema, Fixkosten, Bezugsbasen, Marge) → eigene Sektion <strong>„Herstellkosten"</strong> in der Navigation.</p>
