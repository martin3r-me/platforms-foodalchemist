                    ...
                    @foreach($liste as $l)
                        <tr class="{{ $tr }} {{ $l->active ? '' : 'opacity-50' }}" wire:key="layer-{{ $l->id }}">
                            @if($editId === $l->id)
                                ...
                                <td class="{{ $td }} text-[10px] font-mono text-gray-500">{{ $l->slug }}</td>
                            ...
                            @else
                                <td class="{{ $td }} font-medium text-gray-900">{{ $l->label }}</td>
                                <td class="{{ $td }} text-[10px] font-mono text-gray-500">{{ $l->slug }}</td>
                                <td class="{{ $td }} text-[11px] text-gray-500 max-w-[16rem] truncate">{{ $l->description ?? '—' }}</td>
                                <td class="{{ $td }} text-[11px] text-gray-600">{{ $bindCounts[$l->slug] ?? 0 }}</td>
                                <td class="{{ $td }} whitespace-nowrap">
