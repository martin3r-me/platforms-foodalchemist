...
<div class="space-y-5" data-settings-einsatzorte>
    <div>
        ...
        <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">Bindungs-Ziele fürs Wissen. <b>Bereiche</b> (grob) = ganze KI-Sektion · <b>Prompts</b> (fein) = einzelner KI-Aufruf. Aus der Prompt-Registry abgeleitet (kein Anlegen); Label/Beschreibung + aktiv pflegbar. Gebundenes Wissen wird bei einem Prompt geladen, wenn es an ihn <em>oder</em> seinen Bereich gebunden ist.</p>
    </div>
                    ...
                    @foreach($liste as $l)
                        <tr class="{{ $tr }} {{ $l->active ? '' : 'opacity-50' }}" wire:key="layer-{{ $l->id }}">
                            @if($editId === $l->id)
                                ...
                                <td class="{{ $td }} text-[10px] font-mono text-gray-500 dark:text-gray-400">{{ $l->slug }}</td>
                            ...
                            @else
                                ...
                                <td class="{{ $td }} text-[10px] font-mono text-gray-500 dark:text-gray-400">{{ $l->slug }}</td>
                                <td class="{{ $td }} text-[11px] text-gray-500 dark:text-gray-400 max-w-[16rem] truncate">{{ $l->description ?? '—' }}</td>
                                <td class="{{ $td }} text-[11px] text-gray-600 dark:text-gray-400">{{ $bindCounts[$l->slug] ?? 0 }}</td>
                                <td class="{{ $td }} whitespace-nowrap">
