                    ...
                    @else
                        ...
                        @if($darfEdit)
                            ...
                            <button type="button" wire:click="deleteWg({{ $wg->id }})" wire:confirm="Diese Warengruppe löschen?" @disabled($wg->gp_count > 0)
                                    ...
                                    title="{{ $wg->gp_count > 0 ? 'Wird von GPs genutzt — erst umhängen' : 'löschen' }}">@svg('heroicon-o-trash', 'w-3.5 h-3.5 inline-block align-middle')</button>
                        @endif
                    @endif
                </div>
            @endforeach
        ...
        <div class="flex-1 min-w-0">
            <div class="relative overflow-hidden {{ $card }}" data-subkat x-data="{ dragIdx: null }">
                ...
                <div class="px-5 pt-4 pb-2 flex items-baseline justify-between">
                    <div>
                        <h3 class="font-medium tracking-tight text-gray-900">Sub-Kategorien</h3>
                        <p class="text-[11px] text-gray-500 mt-0.5">Verwaltete Liste je Warengruppe — anlegbar, plus vorhandene GP-Freitextwerte. Rename propagiert auf alle EIGENEN GPs (geerbte bleiben unberührt, D1).</p>
                    </div>
                            ...
                            <tr wire:key="sub-{{ md5($sub->sub_category) }}" class="{{ $tr }}"
                                ...
                                @if($renameAlt === $sub->sub_category)
                                    ...
                                    <td class="{{ $td }} text-right whitespace-nowrap">
                                        <button type="button" wire:click="rename" class="{{ $btnGhostXs }} text-violet-600">Umbenennen</button>
                                        <button type="button" wire:click="$set('renameAlt', null)" class="{{ $btnGhostXs }}">Abbrechen</button>
                                    </td>
                                @else
                                    <td class="{{ $td }} text-gray-700">{{ $sub->sub_category }}</td>
                                    <td class="{{ $td }} text-gray-600">{{ $sub->n }}</td>
                                    <td class="{{ $td }} text-right whitespace-nowrap">
