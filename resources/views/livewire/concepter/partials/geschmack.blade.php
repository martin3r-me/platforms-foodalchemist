            ...
            <div class="space-y-1.5 pt-1">
                @foreach($tasteLabels as $key => $label)
                    ...
                    <div class="flex items-center gap-2">
                        <span class="w-12 text-[11px] text-gray-600 shrink-0">{{ $label }}</span>
                        <div class="flex-1 h-1.5 rounded-full bg-black/[0.06] overflow-hidden">
                            <div class="h-full rounded-full bg-violet-500/70" style="width: {{ $pct }}%"></div>
                        </div>
                        <span class="w-7 text-right text-[10px] text-gray-500 tabular-nums">{{ $pct }}</span>
                    </div>
                @endforeach
            </div>
        </div>
