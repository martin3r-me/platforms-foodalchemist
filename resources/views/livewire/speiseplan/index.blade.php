                ...
                <div class="space-y-0.5 -mx-1">
                    @forelse($plaene as $p)
                        <button type="button" wire:key="sp-{{ $p->id }}" wire:click="waehle({{ $p->id }})"
                            ...
                            <span class="text-[10px] text-gray-500">{{ $p->cycle_weeks }} Wo.-Zyklus · {{ $p->eintraege_count }} Einträge</span>
                        </button>
                    @empty
                        <p class="px-2 py-3 text-[11px] text-gray-500">Noch keine Speisepläne.</p>
                    @endforelse
                </div>
            </div>
        </x-ui-page-sidebar>
            ...
            @if($sp && $kosten)
                <div class="p-4 space-y-3">
                    <div class="text-center py-2">
                        <div class="text-2xl font-semibold text-gray-900 tabular-nums">{{ number_format($kosten['woche']['vk'], 2, ',', '.') }} €</div>
                    ...
                    <div class="pt-2 border-t border-black/5 space-y-1">
                        @if(!empty($wiederholungen))
                            <div class="{{ $label }} text-amber-600">Wiederholungen ({{ count($wiederholungen) }})</div>
                            ...
                            @endforeach
                        @else
                            <p class="text-[11px] text-gray-500 text-center">Keine Wiederholungs-Konflikte.</p>
                        @endif
                    </div>
                </div>
            @else
        ...
        @if($sp)
            ...
            <div class="relative overflow-hidden {{ $card }} p-5 space-y-3" wire:key="sphdr-{{ $sp->id }}">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <button type="button" wire:click="loeschen({{ $sp->id }})" wire:confirm="Speiseplan löschen?" class="{{ $btnGhost }} text-red-600">Löschen</button>
                ...
                @if($ausrollenInfo)<p class="text-[11px] text-violet-600">{{ $ausrollenInfo }}</p>@endif
            </div>

            {{-- Menü-Linien-Editor --}}
            <div class="relative overflow-hidden {{ $card }} p-5 space-y-2" wire:key="splinien-{{ $sp->id }}">
                <div class="flex items-center justify-between">
                    <h3 class="font-medium tracking-tight text-gray-900">Menü-Linien</h3>
                    <span class="text-[11px] text-gray-500">Zeilen der Matrix · pro Plan frei</span>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    @foreach($linien as $linie)
                        <div wire:key="linie-{{ $linie->id }}" class="flex items-center gap-1.5 pl-2 pr-1 py-1 rounded-lg border border-black/5">
                ...
                @if($editLinieId !== null)
                    <div class="mt-1 pt-2 border-t border-black/5 flex flex-wrap items-end gap-2">
                        <div><label class="{{ $label }}">Name</label><input type="text" wire:model="linieForm.name" class="{{ $input }} w-44 h-8" /></div>
                        <div><label class="{{ $label }}">Farbe</label><input type="color" wire:model="linieForm.color" class="h-8 w-12 rounded border border-black/10 bg-transparent" /></div>
            ...
            <div class="flex items-center gap-3 flex-wrap">
                <span class="inline-flex rounded-lg overflow-hidden border border-black/10">
                    @foreach(['woche' => 'Woche', 'monat' => 'Monat'] as $av => $al)
                        <button type="button" wire:click="ansichtSetzen('{{ $av }}')" class="px-3 py-1.5 text-xs {{ $ansicht === $av ? 'bg-violet-500/10 text-violet-700 font-medium' : 'text-gray-600 hover:bg-black/[0.03]' }}">{{ $al }}</button>
                    @endforeach
                </span>
            ...
            @if($ansicht === 'woche')
                ...
                <div class="relative overflow-hidden {{ $card }} p-5">
                        ...
                        <table class="{{ $table }}" style="table-layout:fixed; width:100%; min-width:600px;">
                            <thead><tr class="text-left">
                                ...
                                @foreach($wochenTage as $tag)
                                    <th class="{{ $th }} text-center {{ $tag->isToday() ? 'text-violet-600' : '' }}">{{ $tagKurz[$tag->isoWeekday()] }} <span class="text-gray-500 font-normal">{{ $tag->format('d.m.') }}</span></th>
                                @endforeach
                            </tr></thead>
                            <tbody>
                                ...
                                @foreach($zeilenLinien as $zl)
                                    <tr class="border-t border-black/5 align-top">
                                        <td class="{{ $td }} whitespace-nowrap">
                                            <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full shrink-0" style="background: {{ $zl['color'] ?: '#888780' }}"></span><span class="font-medium text-gray-600">{{ $zl['name'] }}</span></span>
                                        </td>
                                        @foreach($wochenTage as $tag)
                                                ...
                                                <div class="space-y-0.5">
                                                    @foreach($eintraege as $e)
                                                        <div wire:key="e-{{ $e->id }}" class="group flex items-center gap-1 px-1.5 py-0.5 rounded text-[11px]"
                                                            ...
                                                            <button type="button" wire:click="eintragRaus({{ $e->id }})" class="opacity-0 group-hover:opacity-100 text-gray-500 hover:text-red-500 shrink-0">✕</button>
                                                        </div>
                                                    @endforeach
                                                    @if($zl['id'] !== 0)
                                                        <button type="button" wire:click="zelleOeffnen('{{ $ymd }}', {{ $zl['id'] }})" class="w-full text-[11px] text-gray-500 hover:text-violet-500 rounded border border-dashed border-black/10 py-0.5">+</button>
                                                    @endif
                                                </div>
                                            </td>
                                        @endforeach
                                ...
                                @if($zeilenLinien->isEmpty())
                                    <tr><td colspan="6" class="{{ $td }} text-center text-gray-500 text-xs py-4">Oben eine Menü-Linie anlegen, dann Gerichte in die Tage ziehen.</td></tr>
                                @endif
                            </tbody>
                        </table>
                    </div>
                    ...
                    @if($cellDatum !== null)
                        <div class="mt-3 pt-3 border-t border-black/5 space-y-2">
                            <div class="flex items-center gap-2 flex-wrap">
                            ...
                            @elseif($pickerSuche !== '')
                                <p class="text-[11px] text-gray-500">Keine Treffer.</p>
                            @endif
                        </div>
                    @endif
                </div>
                    ...
                    <div style="display:grid; grid-template-columns:repeat(7,minmax(0,1fr)); gap:4px;">
                        ...
                        @for($i = 0; $i < 42; $i++)
                            ...
                            <button type="button" wire:key="cal-{{ $ymd }}" wire:click="tagOeffnen('{{ $ymd }}')"
                                    class="text-left rounded-lg border p-1.5 h-20 transition-colors {{ $imMonat ? 'border-black/5 hover:bg-violet-500/5' : 'border-transparent opacity-40' }} {{ $tag->isToday() ? 'ring-1 ring-violet-400' : '' }}">
                                <div class="flex items-center justify-between">
                                    <span class="text-[11px] {{ $tag->isToday() ? 'text-violet-600 font-semibold' : 'text-gray-600' }}">{{ $tag->format('j') }}</span>
                                    @if($info)<span class="{{ $pill }} {{ $variantPill['secondary'] }} text-[9px]">{{ $info['count'] }}</span>@endif
                                </div>
                                @if($info && $info['vk'] > 0)
                                    <div class="mt-1 text-[10px] text-gray-500 tabular-nums">{{ number_format($info['vk'], 2, ',', '.') }} €</div>
                                @endif
                            </button>
                        @endfor
                    </div>
                    <p class="mt-3 text-[11px] text-gray-500">Tag anklicken → springt in die Wochenansicht. Belegung der Mahlzeit „{{ $mahlzeiten[$mahlzeit] ?? '' }}".</p>
                </div>
            @endif
        @else
