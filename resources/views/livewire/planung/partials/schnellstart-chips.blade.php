        ...
        <div class="flex flex-wrap gap-1.5">
            @foreach($vorlagen as $vid => $v)
                <span @class([
                          ...
                          'border-violet-500 bg-violet-500/10 text-violet-700 ring-1 ring-violet-500/30' => ($aktiveVorlage[$scope] ?? null) === (string) $vid,
                          'border-black/10 bg-black/[0.03] text-gray-600 hover:bg-violet-500/10 hover:text-violet-700' => ($aktiveVorlage[$scope] ?? null) !== (string) $vid,
                    ...
                    <button type="button" wire:click="briefVorlage('{{ $scope }}', '{{ $vid }}')"
                        ...
                        @unless($v['is_global'])<span class="text-amber-500">★</span> @endunless{{ $v['label'] }}
                    </button>
                    @unless($v['is_global'])
                        <button type="button" wire:click="loeschenVorlage('{{ $scope }}', {{ $v['id'] }})"
                                wire:confirm="Vorlage „{{ $v['label'] }}“ löschen?"
                                class="pr-2 pl-0.5 text-[13px] leading-none text-gray-400 hover:text-red-600" title="Eigene Vorlage löschen">&times;</button>
                    @endunless
                </span>
            @endforeach
        </div>
        <p class="text-[11px] text-gray-500 mt-1">Füllt Briefing, Kreativ-Modus und Leitplanken als Startpunkt — alles frei anpassbar. <span class="text-amber-500">★</span> = eigene Vorlage.</p>
    </div>
@endif
