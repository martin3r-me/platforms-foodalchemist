<div class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="px-5 py-4">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    ...
                    <h1 class="text-xl font-semibold tracking-tight text-gray-900">{{ $fb->label }}</h1>
            ...
            <div class="flex flex-wrap items-end gap-4 mt-4">
                ...
                <div class="flex flex-wrap gap-2">
                    <div class="{{ $kpiTile }}"><div class="{{ $kpiTileAccent }}"></div>
                        ...
                        <div class="text-lg font-semibold text-gray-900 tabular-nums">{{ number_format($gesamt['vk_pro_person'], 2, ',', '.') }} €</div>
                    </div>
                    <div class="{{ $kpiTile }}"><div class="{{ $kpiTileAccent }}"></div>
                        <div class="{{ $label }}">EK / Person</div>
                        <div class="text-lg font-semibold text-gray-900 tabular-nums">{{ number_format($gesamt['ek_per_person'], 2, ',', '.') }} €</div>
                    </div>
                    ...
                    @if($gesamt['gesamt_vk'] !== null)
                        <div class="{{ $kpiTile }}"><div class="{{ $kpiTileAccent }}"></div>
                            ...
                            <div class="text-lg font-semibold text-gray-900 tabular-nums">{{ number_format($gesamt['gesamt_vk'], 2, ',', '.') }} €</div>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="px-5 py-3 space-y-3">
            ...
            @if($filter_aktiv)
                <div class="flex items-center gap-3 pt-1">
                    <span class="text-[11px] text-gray-600">{{ $trefferGesamt }} Gericht-Treffer · Preise = ganzes Kapitel (Filter ist Sicht-Linse)</span>
    ...
    @if(count($kapitel) === 0)
        <p class="text-sm text-gray-500 py-10 text-center">Noch keine Kapitel in diesem Foodbook.</p>
    @else
                    ...
                    @foreach($kapitel as $k)
                        <li>
                            <a href="#kap-{{ $k['id'] }}" style="padding-left: {{ $k['depth'] * 12 + 4 }}px"
                               class="block py-1 pr-2 text-xs rounded hover:bg-violet-500/5 truncate {{ $filter_aktiv && $k['n_treffer'] === 0 ? 'text-gray-300' : 'text-gray-600' }}">
                                {{ $k['title'] ?: '(ohne Titel)' }}
                                @if($filter_aktiv)<span class="text-gray-500"> · {{ $k['n_treffer'] }}</span>@elseif($k['agg']['vk_pro_person'] > 0)<span class="text-gray-500"> · {{ number_format($k['agg']['vk_pro_person'], 2, ',', '.') }} €</span>@endif
                            </a>
                        </li>
                    @endforeach
                </ul>
                ...
                <section id="kap-{{ $k['id'] }}" x-data="{ open: true }" class="relative overflow-hidden {{ $card }}" style="margin-left: {{ $k['depth'] * 16 }}px">
                    ...
                    <button type="button" @click="open = !open" class="w-full flex items-center justify-between gap-3 px-5 py-3 text-left">
                        <span class="flex items-center gap-2 min-w-0">
                            <span class="text-gray-500 text-xs transition-transform" :class="open || 'rotate-[-90deg]'">▾</span>
                            <span class="font-medium tracking-tight text-gray-900 truncate">{{ $k['title'] ?: '(ohne Titel)' }}</span>
                            <span class="text-[11px] text-gray-500 shrink-0">{{ $filter_aktiv ? $k['n_treffer'] . ' Treffer' : count($k['bloecke']) }}</span>
                        </span>
                        <span class="flex items-center gap-2 shrink-0 text-xs tabular-nums">
                            <span class="text-gray-600">VK {{ number_format($k['agg']['vk_pro_person'], 2, ',', '.') }} €</span>
                            <span class="text-gray-500">EK {{ number_format($k['agg']['ek_per_person'], 2, ',', '.') }} €</span>
                    ...
                    <div x-show="open" x-cloak class="px-5 pb-3">
                        ...
                        @if($filter_aktiv && $k['n_treffer'] === 0)
                            <p class="text-[11px] text-gray-500 py-2 italic">Keine Treffer in diesem Kapitel.</p>
                        @else
                            @forelse($sichtbareBloecke as $b)
                                <div class="py-1.5 border-t border-black/5 {{ $b['ist_header'] ? 'mt-1' : '' }}">
                                    <div class="flex items-center justify-between gap-3">
                                        <div class="flex items-center gap-2 min-w-0">
                                            ...
                                            @endunless
                                            <span class="truncate {{ $b['ist_header'] ? 'font-semibold text-gray-700' : 'text-gray-800' }}">{{ $b['label'] }}</span>
                                            @if(($b['n_gesamt'] ?? 0) > 0 && $filter_aktiv && $b['n_sichtbar'] < $b['n_gesamt'])
                                                <span class="text-[10px] text-gray-500 shrink-0">{{ $b['n_sichtbar'] }}/{{ $b['n_gesamt'] }}</span>
                                            @endif
                                        </div>
                                        @unless($b['ist_header'])
                                            <div class="flex items-center gap-2 shrink-0 text-xs tabular-nums text-gray-600">
                                                @if($b['pauschal'] > 0)
                                                    <span title="Pauschalpreis">{{ number_format($b['pauschal'], 2, ',', '.') }} € pausch.</span>
                                                @else
                                                    <span title="VK / Person">{{ number_format($b['vk_pp'], 2, ',', '.') }} €</span>
                                                    <span class="text-gray-500" title="EK / Person">EK {{ number_format($b['ek_pp'], 2, ',', '.') }} €</span>
                            ...
                            @empty
                                <p class="text-[11px] text-gray-500 py-2">Keine sichtbaren Blöcke.</p>
                            @endforelse
                        @endif
                    </div>
                </section>
