...
<x-ui-page>
    ...
    <x-ui-page-container padding="px-6 pb-6" spacing="space-y-5">
        ...
        <div class="relative overflow-hidden {{ $card }} px-5 py-4" data-rq-signale>
            ...
            <p class="text-[11px] text-gray-500 dark:text-gray-400 mb-2">Vom System erkannte Auffälligkeiten (Preise, Daten, Marge). „Erledigt" = behoben, „Ignorieren" = bewusst akzeptiert.</p>
                ...
                <div class="flex items-start gap-2 py-1.5 border-t border-black/5 dark:border-white/5 text-[11px]" wire:key="sig-{{ $sig->id }}">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-1.5">
                            <span class="text-gray-500 dark:text-gray-400 shrink-0">{{ $sig->type->label() }}</span>
                            <span class="font-medium text-gray-900 dark:text-gray-100 truncate">{{ $sig->title }}</span>
                        </div>
                        @if($sig->description)<p class="text-gray-600 dark:text-gray-400 mt-0.5">{{ \Illuminate\Support\Str::limit($sig->description, 140) }}</p>@endif
                        @php
                        ...
                        @if($sig->type->value === 'preis_sprung_marge_impact' && $pl)
                            <div class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[10px] text-gray-600 dark:text-gray-400">
                                    ...
                                    @foreach(array_slice($pl['beispiele'], 0, 6) as $bsp)
                                        <a href="{{ route('foodalchemist.verkauf.index', ['rezept' => $bsp['recipe_id']]) }}" wire:navigate
                                           class="text-sky-600 dark:text-sky-400 hover:underline" title="Marge {{ $bsp['marge_pct_alt'] }} % → {{ $bsp['marge_pct_neu'] }} %">
                                            {{ \Illuminate\Support\Str::limit($bsp['name'], 26) }}@if(($bsp['marge_delta_eur'] ?? 0) != 0) <span class="text-gray-500 dark:text-gray-400">({{ number_format($bsp['marge_delta_eur'], 2, ',', '.') }} €)</span>@endif
                                        </a>
                                    @endforeach
                                </div>
                            @endif
            ...
            @empty
                <p class="text-[11px] text-gray-500 dark:text-gray-400">— keine Signale ({{ $signalStatus }}) —</p>
            @endforelse
            <div class="mt-2">{{ $signale->links() }}</div>
        </div>

        <h3 class="text-[11px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400 pt-1">Entscheidungen</h3>
        ...
        <div class="relative overflow-hidden {{ $card }} px-5 py-4" data-rq-matches>
            ...
            <p class="text-[11px] text-gray-500 dark:text-gray-400 mb-2">Aus dem Bulk-Match (exakte Dubletten + Fuzzy) — Übernehmen verknüpft das LA mit dem GP. Zeigt die besten 50.</p>
            @forelse($matches as $m)
                <div class="flex items-center gap-2 py-1 border-t border-black/5 dark:border-white/5 text-[11px]" wire:key="rqm-{{ $m->id }}">
                    ...
                    <span class="text-gray-500 dark:text-gray-400 shrink-0">{{ $m->methode }}</span>
            ...
            @empty
                <p class="text-[11px] text-gray-500 dark:text-gray-400">— nichts offen —</p>
            @endforelse
        </div>
        ...
        <div class="relative overflow-hidden {{ $card }} px-5 py-4" data-rq-bulks>
            ...
            @forelse($bulks as $b)
                <div class="flex items-center gap-2 py-1 border-t border-black/5 dark:border-white/5 text-[11px]" wire:key="rqb-{{ $b->id }}">
                    ...
                    <span class="min-w-0 truncate text-gray-600 dark:text-gray-400" title="{{ is_string($b->value) ? trim($b->value, '"') : '' }}">{{ \Illuminate\Support\Str::limit(trim((string) $b->value, '"'), 60) }}</span>
                    @if($b->confidence !== null)<span class="text-gray-500 dark:text-gray-400 shrink-0">{{ round($b->confidence * 100) }} %</span>@endif
                    <span class="ml-auto shrink-0 flex gap-1">
        ...
        <div class="grid md:grid-cols-3 gap-3">
            <div class="relative overflow-hidden {{ $card }} px-4 py-3" data-rq-vk-ohne-klasse>
                ...
                <p class="text-[10px] text-gray-500 dark:text-gray-400 mb-1.5">V-22-Gate — ✨ Klassifizieren im VK-Panel</p>
                ...
                @empty
                    <p class="text-[11px] text-gray-500 dark:text-gray-400">— keine —</p>
                @endforelse
            </div>
            <div class="relative overflow-hidden {{ $card }} px-4 py-3" data-rq-review>
                ...
                <p class="text-[10px] text-gray-500 dark:text-gray-400 mb-1.5">Freigeben oder zurück in den Entwurf (zeigt 50)</p>
            ...
            <div class="relative overflow-hidden {{ $card }} px-4 py-3" data-rq-ungemappt>
                ...
                <p class="text-[10px] text-gray-500 dark:text-gray-400 mb-1.5">F7.1: Allergene unbekannt, bis gemappt (zeigt 50)</p>
                @forelse($ungemappt as $r)
                    <button type="button" wire:key="rqu-{{ $r->id }}" wire:click="$dispatch('{{ $r->is_sales_recipe ? 'vk-modal.oeffnen' : 'recipe-modal.oeffnen' }}', { id: {{ $r->id }} })"
                        ...
                        <span class="min-w-0 truncate">{{ $r->name }}</span><span class="text-gray-500 dark:text-gray-400 shrink-0 ml-1">{{ $r->n_ingredients_unmapped }}?</span>
                    </button>
                @empty
                    <p class="text-[11px] text-gray-500 dark:text-gray-400">— keine —</p>
                @endforelse
