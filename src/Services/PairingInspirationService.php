...
/**
 * Spec 19 E9.2 — Pairing-Inspiration für die Kreativ-Phase.
 *
...
class PairingInspirationService
{
    ...
    public function inspiration(Team $team, array $seedSlugs, string $modus, int $limitProSeed = 8): array
    {
        ...
        foreach ($seeds as $slug) {
            ...
            // Geerdet: in EINER Query alle tragenden GPs für die Nachbar-Anker holen, nach Anker gruppieren.
            $gpsByAnker = collect();
            if ($geerdet) {
            ...
            $nachbarnOut = $nachbarn->map(function ($n) use ($geerdet, $gpsByAnker) {
                $row = [
                    'anker_id' => (int) $n->id,
                    'slug' => (string) $n->slug,
                ...
                if ($geerdet) {
                    $row['gps'] = ($gpsByAnker->get((int) $n->id) ?? collect())
                        ->map(fn ($g) => [
