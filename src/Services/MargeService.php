...
class MargeService
{
     ...
     *   ek_basis  = ek_per_kg_eur × vk_menge_pro_einheit_g / 1000
     *   vk_netto  = ek_basis × (1 + rohaufschlag_pct/100)        [aufschlag]
     *   vk_brutto = ROUND(vk_netto × (1 + mwst/100), 2)
     *
     * @param  object  $klasse  markup_class-Zeile (rohaufschlag_pct, mwst_satz, formel_typ)
    ...
    public function vkVorschlag(?float $ekPerKgEur, ?float $vkMengeProEinheitG, object $klasse, ?float $mwstSatz = null): ?array
    {
        ...
        $vkNetto = $ekBasis * (1 + ((float) $klasse->rohaufschlag_pct) / 100);
        ...
        return [
            ...
            'formel' => sprintf('VK = EK × (1 + %s%%) · brutto × (1 + %s%% MwSt)',
                rtrim(rtrim(number_format((float) $klasse->rohaufschlag_pct, 2, '.', ''), '0'), '.'),
     ...
     *
     * @return array{vk_netto: ?float, quelle: string, vorschlag: ?array}
     */
    public function effektiverVk(?float $vkNettoManuell, ?float $ekPerKgEur, ?float $vkMengeProEinheitG, ?object $klasse, ?float $mwstSatz = null): array
    {
        ...
        if ($vkNettoManuell !== null && $vkNettoManuell > 0) {
            return ['vk_netto' => $vkNettoManuell, 'quelle' => 'manuell', 'vorschlag' => $vorschlag];
        }

        return ['vk_netto' => $vorschlag['vk_netto'] ?? null, 'quelle' => $vorschlag !== null ? 'klasse' : 'leer', 'vorschlag' => $vorschlag];
    }
}
