...
beforeEach(function () {
    ...
    $this->hg = FoodAlchemistRecipeMainGroup::create(['team_id' => $this->rootTeam->id, 'code' => 'SAU', 'label' => 'Saucen']);
    $this->kat = FoodAlchemistRecipeCategory::create(['team_id' => $this->rootTeam->id, 'main_group_id' => $this->hg->id, 'code' => 'BBQ', 'label' => 'BBQ-Saucen']);
    ...
    $this->mkRezept = fn (string $name, array $extra = []) => FoodAlchemistRecipe::create([
        ...
        'name' => $name, 'status' => 'approved', 'category_id' => $this->kat->id, ...$extra,
    ]);
    $this->bbq = ($this->mkRezept)('BBQ Texas', ['yield_kg' => 0.387, 'ek_total_eur' => 2.17, 'ek_per_kg_eur' => 5.61, 'taste_direction' => 'herzhaft', 'spec_is_vegan' => false]);
    ($this->mkRezept)('Chimichurri', ['taste_direction' => 'herzhaft']);
    ($this->mkRezept)('VK Bowl', ['is_sales_recipe' => true]);                 // D-6: NICHT in der Basis-Sicht
});
...
it('Hauptgruppen-Counts == SQL und basis()-Scope filtert VK-Rezepte raus (DoD M4-04)', function () {
    ...
    expect($svc->hauptgruppenCounts($this->rootTeam))->toBe([$this->hg->id => 2])  // VK Bowl fehlt
        ...
        ->and($svc->paginateBrowser(['taste' => 'herzhaft'], $this->rootTeam)->total())->toBe(2)
