...
it('rendert Zustand ki: KI-Badge, Konfidenz %, Begründung als Tooltip, Reset verdrahtet', function () {
    $html = Blade::render('<x-foodalchemist::ki-header label="Tags" field="tags" quelle="ki" :confidence="0.92" begruendung="Aggregiert aus LAs" />');

    expect($html)->toContain('data-quelle="ki"')
...
it('rendert Zustand manual: Manuell-Badge ohne Konfidenz, Reset bleibt möglich', function () {
    $html = Blade::render('<x-foodalchemist::ki-header label="Tags" field="tags" quelle="manual" />');

    expect($html)->toContain('data-quelle="manual"')
