...
beforeEach(function () {
    ...
    DB::table('foodalchemist_recipe_ingredients')->insert([
        ...
        'menge' => 500, 'einheit_vocab_id' => $g->id, 'position' => 1, 'created_at' => now(), 'updated_at' => now(),
    ]);
...
it('classify: Fake-Roundtrip liefert validierten Vorschlag; Accept schreibt Klasse + Lineage-Trio', function () {
    $this->vk->update(['speisen_klasse_id' => $this->klasse->id]);   // Kontext fürs Echo

    $vorschlag = $this->svc->classify($this->rootTeam, $this->vk->id);
    expect($vorschlag['klasse_id'])->toBe($this->klasse->id)
        ->and($vorschlag['klasse_name'])->toBe('Fleisch')
        ->and($vorschlag['confidence'])->toBe(0.87);

    $this->vk->update(['speisen_klasse_id' => null]);
    $this->svc->acceptKlasse($this->rootTeam, $this->vk->id, $vorschlag['klasse_id'], $vorschlag['confidence'], $vorschlag['begruendung']);

    $this->vk->refresh();
    expect($this->vk->speisen_klasse_id)->toBe($this->klasse->id)
        ->and($this->vk->speisen_klasse_quelle)->toBe('ki')
        ->and((float) $this->vk->speisen_klasse_ai_confidence)->toBe(0.87);
});
...
it('classify: ungültige/fehlende KI-Id ⇒ ehrlicher Nicht-Treffer; Panel macht KEINEN Schreibversuch', function () {
    ...
    expect($this->vk->fresh()->speisen_klasse_id)->toBeNull();
});

it('Override-First: manuell gepflegte Klasse blockt den Accept typisiert', function () {
    $this->vk->update(['speisen_klasse_quelle' => 'manual']);

    expect(fn () => $this->svc->acceptKlasse($this->rootTeam, $this->vk->id, $this->klasse->id, 0.9, null))
...
it('verteileRollen: Echo-Vorschlag validiert gegen Vokabular + Zeilen; Accept schreibt zeilenweise', function () {
    DB::table('foodalchemist_recipe_ingredients')->where('id', $this->zeileId)->update(['rolle' => 'komponente']);  // Kontext-Echo

    $vorschlag = $this->svc->verteileRollen($this->rootTeam, $this->vk->id);
    expect($vorschlag['rollen'])->toBe([$this->zeileId => 'komponente']);

    DB::table('foodalchemist_recipe_ingredients')->where('id', $this->zeileId)->update(['rolle' => null]);
    ...
    expect($n)->toBe(1)                                               // fremde/ungültige Zeilen fallen raus
        ->and(DB::table('foodalchemist_recipe_ingredients')->where('id', $this->zeileId)->value('rolle'))->toBe('aroma_treiber');
});
...
it('Panel-E2E: Klassifizieren → Übernehmen über die Komponente (DoD Fake-Roundtrip + Accept)', function () {
    ...
    expect($this->vk->fresh()->speisen_klasse_quelle)->toBe('ki');
