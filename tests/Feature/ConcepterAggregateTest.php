...
beforeEach(function () {
    ...
    $this->a = $mk([
        'recipe_key' => 'a', 'name' => 'Green Power', 'sales_net' => 2.00, 'ek_total_eur' => 0.60,
        'work_time_min' => 15, 'sales_quantity_per_unit_g' => 250,
        'nutri_kcal_per_100g' => 200, 'nutri_protein_g_per_100g' => 10, 'nutri_fat_g_per_100g' => 5,
        'nutri_carbs_g_per_100g' => 20, 'nutri_salt_g_per_100g' => 1, 'nutri_confidence' => 'high',
        'spec_is_vegan' => true, 'spec_is_vegetarian' => true, 'allergens_confidence' => 'high',
    ]);
    $this->b = $mk([
        'recipe_key' => 'b', 'name' => 'Pulled Pork', 'sales_net' => 3.00, 'ek_total_eur' => 0.90,
        'work_time_min' => 10, 'sales_quantity_per_unit_g' => 200,
        'nutri_kcal_per_100g' => 150, 'nutri_protein_g_per_100g' => 8, 'nutri_confidence' => 'medium',
        'spec_is_vegan' => false, 'spec_is_vegetarian' => false, 'spec_contains_pork' => true,
        'allergens_confidence' => 'low',
    ]);
    $this->c = $mk([
        'recipe_key' => 'c', 'name' => 'Mystery Dish', 'sales_net' => 5.50, 'ek_total_eur' => 1.50,
        'work_time_min' => 20, 'sales_quantity_per_unit_g' => null,        // KEINE Portionsgramm
        'nutri_kcal_per_100g' => 999, 'nutri_confidence' => 'high',        // hat Daten, aber unbrauchbar ohne g
        'spec_is_vegetarian' => true, 'allergens_confidence' => 'medium',
    ]);
});

it('M10R-1: alle additiven Spalten + neuen Tabellen existieren', function () {
    expect(Schema::hasTable('foodalchemist_vocab_classes'))->toBeTrue()
        ->and(Schema::hasTable('foodalchemist_concept_sector_suitability'))->toBeTrue()
        ->and(Schema::hasColumn('foodalchemist_concepts', 'class'))->toBeTrue()
        ->and(Schema::hasColumn('foodalchemist_concepts', 'writing_style_id'))->toBeTrue()
        ->and(Schema::hasColumn('foodalchemist_concepts', 'brief'))->toBeTrue()
        ->and(Schema::hasColumn('foodalchemist_concepts', 'nutrition_cache'))->toBeTrue()
        ->and(Schema::hasColumn('foodalchemist_concepts', 'work_time_min_cache'))->toBeTrue()
        ->and(Schema::hasColumn('foodalchemist_packages', 'class'))->toBeTrue()
        ->and(Schema::hasColumn('foodalchemist_packages', 'nutrition_cache'))->toBeTrue()
        ->and(Schema::hasColumn('foodalchemist_foodbooks', 'writing_style_id'))->toBeTrue();
});
...
it('Paket-Aggregat: Nährwerte/Person aus Portionsgramm, Konfidenz schwächstes Glied', function () {
    ...
    expect($agg['naehrwerte']['kcal'])->toBe(800.0)
        ...
        ->and($agg['naehrwerte']['confidence'])->toBe('medium')        // min(high, medium)
        ->and($agg['work_time_min'])->toBe(25)                      // 15 + 10
        ->and((float) $agg['ek_per_person'])->toBe(1.5)               // 0.60 + 0.90
...
it('Concept-Aggregat: festes Gericht ohne Portionsgramm degradiert ehrlich (Konfidenz, vollstaendig)', function () {
    ...
    $slot1 = $this->concepts->addSlot($this->rootTeam, $c->id, ['role' => 'Vorspeise']);
    $this->concepts->fillSlot($this->rootTeam, $slot1->id, ['package_id' => $p->id]);
    $slot2 = $this->concepts->addSlot($this->rootTeam, $c->id, ['role' => 'Hauptgang']);
    $this->concepts->fillSlot($this->rootTeam, $slot2->id, ['sales_recipe_id' => $this->c->id, 'quantity' => 2]);
    ...
    expect($agg['n_slots'])->toBe(2)
        ...
        ->and($agg['naehrwerte']['confidence'])->toBe('low')           // Lücke deckelt auf „low"
        ->and($agg['work_time_min'])->toBe(45)                      // 15 + 10 + 20 (ohne Mengen-Faktor)
        ->and((float) $agg['ek_per_person'])->toBe(4.5)               // 0.60 + 0.90 + 1.50×2
        ->and($agg['allergene']['contains_pork'])->toBeTrue()
        ->and($agg['allergene']['confidence'])->toBe('low');           // B = schwächstes Glied
});

it('fillSlot persistiert die Aggregat-Caches am Concept (refreshCache-Hook)', function () {
    $p = $this->pakete->create($this->rootTeam, ['name' => 'Salad Wall', 'role' => 'Vorspeise', 'price_mode' => 'manuell']);
    $this->pakete->syncGerichte($this->rootTeam, $p->id, [['sales_recipe_id' => $this->a->id]]);

    $c = $this->concepts->create($this->rootTeam, ['name' => 'Mini']);
    $slot = $this->concepts->addSlot($this->rootTeam, $c->id, ['role' => 'Vorspeise']);
    $this->concepts->fillSlot($this->rootTeam, $slot->id, ['package_id' => $p->id]);

    $c->refresh();
    expect($c->work_time_min_cache)->toBe(15)
        ->and((float) $c->ek_per_person_cache)->toBe(0.6)
        ->and($c->nutrition_cache)->toBeArray()
        ->and($c->nutrition_cache['kcal'])->toEqual(500);            // 200 × 2.5 (JSON normalisiert .0)
});
...
it('syncGerichte persistiert den Paket-Nährwert-Cache', function () {
    ...
    $p->refresh();
    expect($p->work_time_min_cache)->toBe(15)
        ->and($p->nutrition_cache)->toBeArray()
        ->and($p->nutrition_cache['kcal'])->toEqual(500);           // JSON normalisiert .0
});
...
it('EK teilt durch die Portionszahl (Batch→Portion), nicht ek_total roh', function () {
    ...
    $batch = FoodAlchemistRecipe::create([
        'team_id' => $this->rootTeam->id, 'status' => 'approved', 'is_sales_recipe' => true,
        'recipe_key' => 'batch', 'name' => 'Gulasch (Topf)', 'sales_net' => 5.00,
        'ek_total_eur' => 30.00, 'sales_unit_count' => 10,
    ]);

    $c = $this->concepts->create($this->rootTeam, ['name' => 'Batch-Test']);
    $slot = $this->concepts->addSlot($this->rootTeam, $c->id, ['role' => 'Hauptgang']);
    $this->concepts->fillSlot($this->rootTeam, $slot->id, ['sales_recipe_id' => $batch->id, 'quantity' => 2]);

    $agg = $this->agg->conceptAggregat($c->refresh());

    expect((float) $agg['ek_per_person'])->toBe(6.0)          // 30 ÷ 10 × 2 (nicht 60)
    ...
    expect((float) $cockpit['ek_per_person'])->toBe(6.0)
...
it('Gramm-Einheit rechnet anteilig am Portionsgewicht', function () {
    ...
    $beilage = FoodAlchemistRecipe::create([
        'team_id' => $this->rootTeam->id, 'status' => 'approved', 'is_sales_recipe' => true,
        'recipe_key' => 'beilage', 'name' => 'Reis', 'sales_net' => 2.00,
        'ek_total_eur' => 4.00, 'sales_unit_count' => 1, 'sales_quantity_per_unit_g' => 250,
    ]);

    $c = $this->concepts->create($this->rootTeam, ['name' => 'Gramm-Test']);
    $slot = $this->concepts->addSlot($this->rootTeam, $c->id, ['role' => 'Beilage']);
    $this->concepts->fillSlot($this->rootTeam, $slot->id, ['sales_recipe_id' => $beilage->id]);
    $this->concepts->setSlotMengeEinheit($this->rootTeam, $slot->id, 125, $gramm->id);

    $agg = $this->agg->conceptAggregat($c->refresh());

    expect((float) $agg['ek_per_person'])->toBe(2.0)          // 4 × (125 ÷ 250)
...
it('Gramm-Einheit ohne Portionsgewicht trägt ehrlich nicht bei', function () {
    ...
    $ohneG = FoodAlchemistRecipe::create([
        'team_id' => $this->rootTeam->id, 'status' => 'approved', 'is_sales_recipe' => true,
        'recipe_key' => 'noweight', 'name' => 'Mystery', 'sales_net' => 9.00,
        'ek_total_eur' => 4.00, 'sales_unit_count' => 1, 'sales_quantity_per_unit_g' => null,
    ]);

    $c = $this->concepts->create($this->rootTeam, ['name' => 'Gramm-Lücke']);
    // 1 belastbare Portion + 1 Gramm-Position ohne Portionsgewicht.
    $s1 = $this->concepts->addSlot($this->rootTeam, $c->id, ['role' => 'Hauptgang']);
    $this->concepts->fillSlot($this->rootTeam, $s1->id, ['sales_recipe_id' => $this->a->id, 'quantity' => 1]); // a: ek 0.60
    $s2 = $this->concepts->addSlot($this->rootTeam, $c->id, ['role' => 'Beilage']);
    $this->concepts->fillSlot($this->rootTeam, $s2->id, ['sales_recipe_id' => $ohneG->id]);
    $this->concepts->setSlotMengeEinheit($this->rootTeam, $s2->id, 100, $gramm->id);

    $agg = $this->agg->conceptAggregat($c->refresh());

    expect((float) $agg['ek_per_person'])->toBe(0.6)          // nur a trägt bei, Mystery fällt ehrlich raus
...
it('Gewicht/Person: Σ Effektiv-Gramm; Position ohne Portionsgewicht → unvollständig', function () {
    ...
    $s1 = $this->concepts->addSlot($this->rootTeam, $c->id, ['role' => 'Vorspeise']);
    $this->concepts->fillSlot($this->rootTeam, $s1->id, ['sales_recipe_id' => $this->a->id, 'quantity' => 1]); // 1 × 250 g
    $s2 = $this->concepts->addSlot($this->rootTeam, $c->id, ['role' => 'Hauptgang']);
    $this->concepts->fillSlot($this->rootTeam, $s2->id, ['sales_recipe_id' => $this->b->id, 'quantity' => 2]); // 2 × 200 g
    ...
    $x1 = $this->concepts->addSlot($this->rootTeam, $c2->id, ['role' => 'Vorspeise']);
    $this->concepts->fillSlot($this->rootTeam, $x1->id, ['sales_recipe_id' => $this->a->id, 'quantity' => 1]); // 250 g
    $x2 = $this->concepts->addSlot($this->rootTeam, $c2->id, ['role' => 'Extra']);
    $this->concepts->fillSlot($this->rootTeam, $x2->id, ['sales_recipe_id' => $this->c->id, 'quantity' => 1]); // c: kein Portionsgewicht
...
it('Stück-Modus (kg↔Stück): EK/Stück = ek_total÷ertrag, Gewicht/Stück = yield_g÷ertrag', function () {
    ...
    $toertchen = FoodAlchemistRecipe::create([
        'team_id' => $this->rootTeam->id, 'status' => 'approved', 'is_sales_recipe' => false,
        'recipe_key' => 'toertchen', 'name' => 'Törtchen-Teig',
        'ek_total_eur' => 20.00, 'yield_kg' => 20, 'yield_pieces' => 50,
    ]);

    $c = $this->concepts->create($this->rootTeam, ['name' => 'Stück-Test']);
    $slot = $this->concepts->addSlot($this->rootTeam, $c->id, ['role' => 'Petit Four']);
    $this->concepts->fillSlot($this->rootTeam, $slot->id, ['sales_recipe_id' => $toertchen->id, 'type' => 'basisrezept', 'quantity' => 2]);
    $this->concepts->setSlotMengeEinheit($this->rootTeam, $slot->id, 2, $stk->id);

    $agg = $this->agg->conceptAggregat($c->refresh());
    expect((float) $agg['ek_per_person'])->toBe(0.8)            // 20/50 × 2
