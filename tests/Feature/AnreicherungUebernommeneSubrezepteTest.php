 ...
 *
 * Zwei der drei Ursachen — die Anreicherungs-Seite:
 *
 ...
 *
 * Der Wissens-Recall (sachfremde Dossiers) und der GP-Pick (`Tomaten: TK, getrocknet` mit
 * 1.600 g) sind derselben Sitzung entstammt, gehoeren aber in eigene Plaene — sie stehen auf
 * `feat/wissen-recall-und-gp-pick` samt Tests bereit.
 */
beforeEach(function () {
...
it('A8: nach erfolgreicher Anreicherung wird der Reifegrad NACHGEZOGEN — sonst haengt der Lauf ewig in review', function () {
    // Die Gegenprobe zum eigenen Fix: der Reuse-Marker wird bei `firstOrCreate` NICHT
    // aktualisiert, wenn der Step schon existiert. Bliebe er auf `reif => false`, hätte der
    // Fix die Lüge nur umgedreht — die Zeile behauptete für immer „Bestand unfertig" und
    // `recomputeRunStatus` käme nie über `review` hinaus.
    $gp = $this->makeGp($this->rootTeam, 'Karotten');
    $bruehe = $this->makeRecipe($this->rootTeam, 'Basisrezept: Gemüsebrühe', ['status' => 'draft', 'ek_total_eur' => 2.08]);
    ($this->zutat)($bruehe, $gp->id, 150, ($this->gramm)(), 'Karotten');

    $run = FoodAlchemistCascadeRun::create([
        'team_id' => $this->rootTeam->id, 'scope' => 'rezept', 'status' => 'review', 'staged' => true,
    ]);
    $step = FoodAlchemistCascadeRunStep::create([
        'team_id' => $this->rootTeam->id, 'cascade_run_id' => $run->id, 'kind' => 'rezept',
        'status' => 'skipped', 'ref_type' => 'recipe', 'ref_id' => $bruehe->id, 'depth' => 1,
    ...
    // Die Lücke schließen (das tut die Anreicherung fachlich), dann den Job laufen lassen.
    ($this->schritt)($bruehe, 'Gemüse ansetzen und ziehen lassen.');

    (new EnrichRecipeJob($this->rootTeam->id, (int) auth()->id(), (int) $bruehe->id, null, false, (int) $step->id, false, false, false))
        ->handle(app(\Platform\FoodAlchemist\Services\RecipeOneShotService::class));

    expect($step->fresh()->deferred['reuse']['reif'] ?? null)->toBeTrue()
        ->and($step->fresh()->deferred['reuse']['luecken'] ?? null)->toBe([])
