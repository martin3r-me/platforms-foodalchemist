...
beforeEach(function () {
    ...
    app(TeamSettingsService::class)->update($this->rootTeam, [
        'hk2_zuschlag_pct' => 20, 'stundensatz_eur' => 30, 'marge_pct' => 15,
    ]);
});
...
it('Gericht-Position zieht Wareneinsatz + Arbeitszeit pro Portion als Snapshot', function () {
    ...
    expect($p->einheit)->toBe('Portion')
        ->and((float) $p->einzel_ek)->toBe(5.0)
        ->and((int) $p->arbeitszeit_min)->toBe(10)
...
it('Basisrezept-Position zieht €/kg, GP-Position rechnet in kg', function () {
    $b = FoodAlchemistRecipe::create([
        ...
        'ist_verkaufsrezept' => false, 'ek_per_kg_eur' => 8.00,
    ]);
    $k = $this->svc->create($this->rootTeam, 'Test');
    $p = $this->svc->addPosition($this->rootTeam, $k->id, 'basisrezept', $b->id);

    expect($p->einheit)->toBe('kg')->and((float) $p->einzel_ek)->toBe(8.0)
        ->and($p->arbeitszeit_min)->toBeNull();
});
...
it('berechne: HK1 = Σ Wareneinsatz, Lohn aus Arbeitszeit-Rollup, HK2 + Marge-Override', function () {
    ...
    $this->svc->addPosition($this->rootTeam, $k->id, 'gericht', $g->id, ['menge' => 10]);
    $this->svc->addPosition($this->rootTeam, $k->id, 'basisrezept', $b->id, ['menge' => 2]);
    $this->svc->addPosition($this->rootTeam, $k->id, 'frei', null, ['label' => 'Deko', 'einzel_ek' => 3, 'menge' => 1]);
    ...
    expect($r['hk1'])->toBe(69.0)
        ->and($r['arbeitszeit_min'])->toBe(100.0)
...
it('Position aktualisieren + entfernen', function () {
    ...
    $p = $this->svc->addPosition($this->rootTeam, $k->id, 'frei', null, ['label' => 'X', 'einzel_ek' => 10, 'menge' => 1]);

    $this->svc->updatePosition($this->rootTeam, $p->id, ['menge' => '3', 'einzel_ek' => '4,50']);
...
it('quellen: Gerichte vs Basisrezepte getrennt, nach Name filterbar', function () {
    FoodAlchemistRecipe::create(['team_id' => $this->rootTeam->id, 'recipe_key' => 'v', 'name' => 'Verkauf A', 'status' => 'approved', 'ist_verkaufsrezept' => true]);
    FoodAlchemistRecipe::create(['team_id' => $this->rootTeam->id, 'recipe_key' => 'x', 'name' => 'Basis B', 'status' => 'approved', 'ist_verkaufsrezept' => false]);
