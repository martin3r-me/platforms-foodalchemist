 ...
 *
 * warengruppe_code NULL = globaler Stamm-Lieferant (Quelle stamm_lieferant, 21),
...
return new class extends Migration
{
    ...
    {
        Schema::create('foodalchemist_stamm_lieferanten', function (Blueprint $table) {
            $table->id();
            $table->uuid('uuid')->unique();
            $table->unsignedBigInteger('team_id')->index();
            $table->foreignId('supplier_id')->constrained('foodalchemist_suppliers')->cascadeOnDelete();
            $table->string('warengruppe_code', 8)->nullable()->index()->comment('NULL = globaler Stamm');
            $table->timestamps();
            $table->softDeletes();
    ...
    {
        Schema::dropIfExists('foodalchemist_stamm_lieferanten');
    }
};
